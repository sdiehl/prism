...
0. **The World is an Effect.** The unfortunate existence of reality intrudes on our pure functional fun, so the best we can do is constrain the world's heterogeneous effects and categorize them in a principled way. Prism makes that compromise explicit: effect rows delimit the world's authority, totality evidence says whether a computation comes back, traces pin what crossed the boundary, content hashes name the program and the artifacts that emerged, and lineage records the causal path between them. Once every intrusion from the real world is typed, pinned, or named, the result is finally _truly_ pure. And finally then our programs can safely never be run. [^p-accountability]
1. **Determinism is the contract.** Every observable behavior of a compiled program is a pure function of the source and the pinned input trace. The effect-lowering strategy, the optimization level, the backend, the scheduler, and the compiler's own prior workload are cost decisions and must be unobservable. Replay, content addressing, and cross-backend attestation are corollaries of this one theorem, not features beside it.[^p-determinism]
2. **One semantic pipeline, whole-program.** Modules are checked independently for incrementality, but they compose into one merged program before the middle end, so specialization, effect strategy, closure planning, and reachability range over everything at once. Module artifacts join the whole-program path; they never route around it.[^p-wholeprogram]
3. **Representations carry the design.** Each phase lowers into a representation that can express strictly less than the one above it: sugar becomes unrepresentable after desugaring, unresolved names after resolution, implicit choices after checking. A decision made once is never reopened downstream, and a missed lowering is a compiler type error rather than a runtime surprise.[^p-representations]
4. **The core is call-by-push-value in A-normal form.** Values and computations are syntactically distinct, so every effect operation, allocation, and evaluation-order decision sits in a distinguished position. This is what makes the middle-end passes tractable and is where the language's meaning and its content-addressed identity are fixed.[^p-cbpv]
5. **The checker decides; elaboration transcribes.** Bidirectional type and effect inference resolves every implicit choice, records it as a checked fact keyed by node, and no later pass makes a type-system judgment again. Effects are principal row types solved by unification, never silently widened.[^p-checker]
6. **The middle end carries witnesses and is independently verified.** Transformations run over a typed Core whose phase markers make illegal stagings unrepresentable, and an independent verifier re-checks scope, type, effect, handler, and reuse witnesses at every phase transition. This is translation validation: verify each output, rather than prove each pass.[^p-verify]
7. **Effect lowering is a cost cascade, invisible by construction.** Handlers compile through a ladder of strategies from full erasure through evidence passing down to the free monad, and the tier chosen must never be observable; a differential oracle forces slow tiers and diffs them against fast ones. Capability effects make the input trace explicit, which is what keeps record and replay total.[^p-effects]
8. **Memory is garbage-free reference counting with reuse.** Ownership is inserted as explicit dup/drop operations, verified balanced on every path, and matching allocations reuse cells in place, so functional updates compile to mutation without a tracing collector.[^p-perceus]
9. **Identity is content-addressed; incrementality is memoization.** Definitions, interfaces, and artifacts are named by hashes of their semantic content, so an edit invalidates exactly its dependency closure and a build is a demand-driven query graph with early cutoff.[^p-content]
10. **Backends render; the emitter decides.** One shared emitter owns every semantic decision, and a backend is only instruction spelling behind a small trait, so targets cannot drift apart semantically and their agreement is a continuous cross-check. The interpreter, consuming pre-lowering Core, is the reference oracle every native backend must match byte for byte.[^p-backends]
11. **The store remembers; it does not decide.** A cached artifact may accelerate the canonical pipeline only when its identity and phase invariants validate. A cache hit can reuse an answer; it can never become a second semantic authority or invent an answer the uncached compiler would not produce.
12. **Every guarantee names its witness.** A phase type, an independent verifier, an interpreter comparison, a Lean theorem, a solver receipt, a digest, and a replay certificate establish different things. Every claim says which witness supports it, what that witness checked, and what remains trusted; no claim is allowed to grow stronger in the retelling.
13. **Nothing is ambient; every dependency is data.** The world a program touches is spelled in its row; the moments it observes are frames in its trace; the identity of a definition is the hash of its semantic content; the origin of every artifact is an edge in its lineage. What is not declared cannot happen within the guarantee; what happened can be replayed; what exists can be named; what was produced can be traced.[^p-no-ambient]
14. **The compiler should be fun to hack on.** If it is not fun, what is the point? Prism does not need to be useful or do anything in particular; it just needs to be fun.

[^p-accountability]: "Effect" in this principle is the broad semantic sense, not a claim that determinism and termination are surface row labels. Prism realizes the account through separate mechanisms: the effect row bounds permitted authority, totality analysis records what is known about return, the trace fixes actual external observations, deterministic semantics relates those inputs to one result, hashes name exact content, and lineage records the claimed relation and whatever validation or replay evidence was actually obtained. A digest alone does not prove behavior; each part is weaker alone, and lineage must say honestly when an edge was recorded, rehashed, structurally checked, or replay-verified.

[^p-determinism]: The oracle discipline is differential testing ([McKeeman, 1998](bibliography.md#mckeeman-1998)); the replay-as-contract stance is shared with record-and-replay debuggers such as rr and with deterministic-simulation testing as practiced by FoundationDB.

[^p-wholeprogram]: Whole-program compilation as the semantic baseline follows MLton ([Weeks, 2006](bibliography.md#weeks-2006)) and Stalin, Prism's two direct compiler precursors; the join of separately checked modules into one optimized program deliberately trades separate codegen for cross-module truth.

[^p-representations]: Phase-indexed syntax follows the "Trees that Grow" technique ([Najd & Peyton Jones, 2017](bibliography.md#najd-peytonjones-2017)); the discipline of making illegal states unrepresentable at each stage is the ML tradition's, and GHC's Core-after-desugar boundary is the direct precedent.

[^p-cbpv]: Call-by-push-value is Levy's calculus ([Levy, 2004](bibliography.md#levy-2004)); A-normal form is from [Flanagan et al. (1993)](bibliography.md#flanagan-1993).

[^p-checker]: Bidirectional higher-rank inference follows [Dunfield & Krishnaswami (2013)](bibliography.md#dunfield-krishnaswami-2013), with local type inference tracing to [Pierce & Turner (2000)](bibliography.md#pierce-turner-2000). Principal row-typed effects follow Koka ([Leijen, 2017](bibliography.md#leijen-2017)). Type classes compile by dictionary passing ([Peterson & Jones, 1993](bibliography.md#peterson-jones-1993)); pattern matches compile to decision trees ([Maranget, 2008](bibliography.md#maranget-2008)).

[^p-verify]: The lint lineage is GHC's Core Lint; the verify-each-output stance is translation validation ([Pnueli, Siegel & Singerman, 1998](bibliography.md#pnueli-1998); [Necula, 2000](bibliography.md#necula-2000)).

[^p-effects]: Algebraic effects and handlers descend from [Plotkin & Pretnar (2009)](bibliography.md#plotkin-pretnar-2009); the evidence-passing compilation is [Xie et al. (2020)](bibliography.md#xie-2020) and [Xie & Leijen (2021)](bibliography.md#xie-leijen-2021), and the multi-strategy cascade with a free-monad floor ([Kiselyov & Ishii, 2015](bibliography.md#kiselyov-ishii-2015)) is Prism's arrangement of Koka's toolkit.

[^p-perceus]: Garbage-free reference counting with reuse is Perceus ([Reinking et al., 2021](bibliography.md#reinking-2021)), with frame-limited reuse ([Lorenzen & Leijen, 2022](bibliography.md#lorenzen-leijen-2022)); the in-place discipline continues in FP^2 ([Lorenzen, Leijen & Swierstra, 2023](bibliography.md#lorenzen-fp2-2023)); the destination-passing tail lowering is tail recursion modulo cons ([Bour, Clément & Scherer, 2021](bibliography.md#bour-clement-scherer-2021)).

[^p-content]: Content-addressed definition identity is Unison's founding idea; the demand-driven query graph with early cutoff follows the incremental architecture of rustc's query system and Salsa, and the immutable content-addressed store echoes Nix.

[^p-backends]: The one-emitter-many-spellings seam is Prism's own, spelling into LLVM ([Lattner & Adve, 2004](bibliography.md#lattner-adve-2004)); the reference-interpreter-as-oracle discipline is differential compiler testing ([McKeeman, 1998](bibliography.md#mckeeman-1998); [Yang et al., 2011](bibliography.md#yang-2011) is the modern exemplar).

[^p-no-ambient]: A definition's “behavior hash” is the digest of its canonical elaborated Core and semantic metadata, not a proof that extensionally equivalent programs share a hash. Replay likewise requires a recorded trace. The principle is that an influence cannot remain unnamed while still being covered by Prism's guarantee, not that reality has ceased to contain surprises.
...
| [Codegen](#backends)                                  | optimized core to the interpreter; lowered core to LLVM or MLIR                                                               | target choice changes cost, not behavior          |
...
| `optimizer-facts`       | the per-definition optimizer fact sheet as versioned JSON     |
...
Both layers also exist a second time, written in Prism, as `Syntax.Lex` and `Syntax.Layout`. They are diffed against this implementation rather than substituted for it, and the [open-compiler](#open-compiler) section describes what that buys and what it does not.
...
There are two parser implementations. Rust/LALRPOP remains the stage-0 compiler parser and comparison oracle. The Prism standard library contains the parallel `Syntax.Parse` implementation, with program, expression, type, pattern, declaration, layout, and diagnostic entry points. Its Pratt expression kernel and generated Type, Pattern, and control families are exercised by the self-parse and differential corpus, but the Rust parser still owns production compilation while parity and diagnostics converge.

The parser consumes the lexer's virtual layout tokens and produces the lossless syntax artifacts used by the open-compiler surface. Type and parse errors are structured diagnostics before they are rendered as a source caret. This tiny source is also used throughout the compiler examples, so it is a useful smoke test for both parser implementations:
...
```prism,ignore
match s of
  Line(0, _) => 1          -- one run of three arms from one source arm,
  Line(1, _) => 1          -- enumerated leftmost alternative first, each
  Ring(0) => 1             -- carrying the whole source arm's span
  _ => 0
```
...
The enclosing `Checked` value is a sealed owner. Consumers retain field-based shared reads through its `CheckedView`, but cannot construct the owner or obtain mutable access to its related tables. Only checker-owned construction and narrow compiler transitions may update diagnostics or presentation-only tooltip facts, so a downstream phase cannot forge a checked aggregate while preserving the existing query surface.
...
An executable build first closes the checked program over its entry point. Every imported or prelude definition and instance that `main` cannot reach, through a name occurrence or through the dictionary evidence the checker recorded at a dispatch site, is dropped before this traversal, while the root file's own definitions and every type, effect, class, and alias declaration stay. The checker has already judged the whole program by then, so no diagnostic is lost; the identity, check, and report surfaces elaborate the whole namespace they are asked about.
...
An independent verifier then proof-checks the completed typed artifact without unification or inference. Typed builders and rewrites produce explicit unchecked assemblies; only successful whole-program verification can mint the opaque phase marker that crosses the erasure boundary back into executable Core. SCC fragments remain unchecked until regrouped, where the global-name set is also revalidated. Steps that already own the complete verification environment may perform this transition internally. Effect lowering retains that environment and its constructor table as one closed artifact: every callback result is re-audited under the retained environment, and only the artifact may erase and validate the final stage. The erased pipeline's `ElaboratedCore` and `LoweredCore` wrappers follow the same consume-and-validate rule: their public doors run the stage lint, and adding synthesized functions consumes and revalidates the wrapper, so a checked claim cannot become stale. Environment, construction, and verification failures use the compiler's structured error variants and canonical internal codes `E9995`, `E9996`, and `E9997`, while erasure and specialization compatibility failures use `E9994` and `E9993`. Callers therefore never need to interpret an opaque error string.
...
**The free-monad fallback** applies when an effect escapes static tracking: buried in data, dynamically applied, masked, genuinely multishot (a clause that resumes `k` more than once), or self-referential (a handler whose own body performs the effect it handles). A callback hidden in an aggregate is judged by its stored thunk row as well as its concrete producer, because pattern extraction carries no flow signature. That stored row is a claim the extraction must honor: a concrete label, or a row chosen elsewhere (a free variable or an existential), calls the recovered thunk at the declared convention even when the lambda inside performs less, while a row the stored function itself quantifies is ordinary polymorphism each use site instantiates and stays direct. Representation wrappers are the evidence for such a widening, so the judgment reads through them rather than following their operands blind. Only a callback whose claim cannot be met that way reaches the uniform fallback. A declared row nothing in the container ever performs stays legal on the same reasoning: each element satisfies the annotation by subsumption, so the program compiles with the ordinary unused-effect warning and the unwitnessed row costs at most a slower rung, never an error. A multishot handler forces this path because the two fast paths erase `k`, and a continuation invoked more than once must exist as a reusable value. Here the delimited continuation is reified in full: each computation becomes a tree of `EPure` and `EOp` cells threaded by `ebind` (shown below), and the continuation each `EOp` still owes is an explicit field a clause can hold, drop, or apply repeatedly.
...
Reference counting runs after effect lowering, over the handler-free core, so it counts evidence parameters and any reified cells as ordinary values. Memory is managed by Perceus-style reference counting ([Reinking et al., 2021](bibliography.md#reinking-2021)): every parameter and binding is owned and consumed exactly once on every control-flow path from its binding to the end of its scope; a second use inserts a `dup` and an unused value inserts a `drop`. Perceus places these operations precisely rather than conservatively at scope exit, which frees a cell at the earliest point the reuse pass below can claim it. Insertion carries ownership partitions, borrow masks, lazy case-arm renames, and value/thunk reconstruction on explicit frames, retaining the same source-order fresh-name sequence without inheriting Core depth from the caller. Closure captures are borrowed (read without being consumed) and duplicated before a consuming use, as is a `borrow` parameter (see [declarations and programs](./spec.md#declarations-and-programs)). The parameters a function borrows are recorded as a per-function bit vector, its interprocedural **borrow signature**, which every caller consults to place its `dup`/`drop` correctly. Those signatures are inferred as well as declared: a call-graph fixpoint marks every parameter a provably pure function only reads, so a read-only traversal loans its argument across the call rather than threading a retain and release pair through it. Discovery uses the canonical iterative visitor, loan inference carries environments and delayed fixpoint updates on explicit frames, and tail recognition classifies a borrowed virtually reassociated arena. Allocation-certificate and linearity checks likewise reduce nested computations and values over explicit postorder worklists, preserving path joins, scopes, captures, call certificates, and diagnostic order without using stack growth. Inference augments the declared annotations at this boundary alone and is a pure function of the checked source, so like a lowering tier it is a cost decision the program's observable behavior cannot reveal, never part of a definition's identity. Because that signature crosses call sites, it is one of the analyses that complicates the move to separate compilation (see [name resolution and modules](#name-resolution-and-modules)).

The reuse pass then turns drops into in-place updates. When a uniquely owned scrutinee is dropped and the continuation rebuilds a constructor of the same or smaller size, the `drop` becomes a scoped reuse node, `WithReuse { token, freed, body }`: it frees the cell once and binds a **reuse token** over the continuation, and the rebuild spends that token with an in-place `Reuse(token, ctor)`, so `map` and tree rebuilds mutate the spine in place. Selection records the all-or-nothing branch decision in a flat plan arena, then an explicit continuation worklist consumes the owned input and reconstructs it without recursive descent. The token is a binder that only a `Reuse` may name, and the rewrite spends it on every control path or declines wholesale (keeping the safe no-reuse body), so freeing a cell once and spending its token at exactly one allocation are well-formedness properties of the term rather than a condition checked afterward.

An independent verifier re-checks that output. `fbip::balanced` re-simulates the inserted `dup`, `drop`, and reuse operations as a linear-token machine: each owned binding starts with one token, a `dup` adds one and a `drop` or consuming use removes one, a use may never drive the count below zero, every binding must reach zero before leaving scope, the two arms of a branch must agree, and a `WithReuse` grants its token exactly one credit the body must spend. The simulation carries scopes, branch environments, case joins, and nested thunk checks on one explicit worklist, so a deep computation or value does not inherit stack depth from its caller. It runs over the reference-counted core on every interpreter entry and across the whole example and test corpus, so an under-`dup`, an over-`drop`, or an unbalanced branch left by the insertion pass surfaces as an internal error rather than a leak or a double free at run time. Core Lint adds the dual direction under `PRISM_CORE_LINT` (see [lint, telemetry, and parity](#lint-telemetry-and-parity)): its reuse-token overspend check is an explicit path-sensitive worklist, covering the fault the balance check does not see without inheriting tree depth from its caller.

The `fip`/`fbip` annotations (see [declarations and programs](./spec.md#declarations-and-programs)) are the fully-in-place discipline of [Lorenzen et al. (2023)](bibliography.md#lorenzen-fp2-2023), here static checks layered on these passes. `fbip` proves zero fresh allocation and a call-graph closure over annotated, allocation-free callees. `fip` adds two further properties: linearity (each owned binding is consumed at most once, checked on the source term, with scalars exempt because adjusting the count of an unboxed word costs nothing) and bounded stack. Tail-call and tail-modulo-constructor/addition classification follows virtual bind reassociation, branches, cases, reuse scopes, and nested values with explicit worklists. It is shared with codegen, so an accepted `fip` function always lowers to a loop; acceptance never outruns what the backend emits.

`@ bounded_stack` exposes that stack judgment independently. The checker uses the same direct-call graph, strongly connected components, and tail-recursion classification as `fip`, but permits allocation and non-linear uses. Every member of a recursive component and every user-defined direct callee must carry `@ bounded_stack` or `fip`; known constant-stack primitives are admitted, while an indirect call is rejected because its callable type has no stack certificate. A failure names the first uncertified callee or recursive obstruction. Passing claims are erased before Core, so the annotation changes acceptance rather than generated behavior.

`@ linear` exposes the ownership judgment independently over raw Core, before reference counting inserts the `dup` and `drop` operations that realize ownership. Its type-aware walk permits each owned non-immediate parameter, pattern field, local result, and closure capture at most one consuming use on a control path, while tagged immediates are unrestricted. Borrowed parameters fail the claim. Direct calls that receive an owned value must target an `@ linear` or `fip` function; indirect calls decline conservatively because callable types do not yet carry the certificate. The check shares the `fip` linearity implementation but does not constrain allocation or stack growth, and its passing claim erases before Core.
...
The emitter and `Isa` split responsibilities at one boundary. The emitter walks the fully-lowered Core (after effect lowering and reference counting) and, node by node, mints an SSA operand _name_ for each result and drives `Isa` with those names; `Isa` never sees a Core node, and no third IR sits between the two. Codegen is therefore a single Core walk that emits a stream of instruction calls: `Comp`/`Value` in, `String` operand names threaded through a register map, `Isa` calls out, target text at the leaves. Every Core-level judgment (evaluation order, allocation, reference counting, tail-call and reuse classification) is made once in that walk, above `Isa`, which is why a backend inherits all of it and spells only instructions.
...
The compiler describes this contract with one `TypeLayout` query whose independent facets are local storage, function-boundary ABI, zero-word possibility, reference-count behavior, and, for a literal, whether it mints a cell per use or shares one interned into the program image. They are kept apart rather than collapsed into a single overloaded enum because they genuinely disagree: `Unit` and `Bool` are both immediate words but only `Unit` is zero, and an unboxed product is multiword locally yet boxed the moment it crosses the native ABI. Unknown or non-executable layouts fail closed; nominal types require declaration evidence, preserved through module interfaces even when constructors are hidden, before they may be treated as non-null cells because mandatory newtype erasure can expose the wrapped representation.
...
A region cell carries an arena-owned marker in its reference-count word, kept out of the tag word so match dispatch, which compares tag words directly, never sees it. `prism_rc_inc`, `prism_rc_dec`, the child-scan decrement, and the reuse-token path all treat a marked cell inertly, so `dup`/`drop` are no-ops inside a region, the region stays the cell's sole owner, and no `rc == 1` uniqueness fast path ever mistakes a shared region cell for unique. Each region cell is also threaded onto an intrusive per-region list. At `arena_exit` the runtime deep-promotes every arena-owned cell reachable from the activation's result into ordinary reference-counted cells (so a value may outlive its region for the cost of a copy, never a use-after-free), walks that list to release the reference-counted children the region's cells own, and returns the whole block chain in one pass. The promotion walk forwards as it copies: a copied region cell records its copy's address in the tag word it is about to abandon, so a cell reached along several paths is copied once and the one copy shared. Sharing inside the region therefore survives promotion instead of being expanded into a tree, which is the difference between linear and exponential cost when the escaping value is a diamond. The walk is iterative over an explicit worklist, like the child-scan decrement, so depth costs no C stack.
...
Five environment-gated counters report to stderr at exit, leaving program stdout untouched. `PRISM_CHECK_LEAKS` reports the live-cell balance, which a clean run drives to zero. `PRISM_ALLOC_STATS` reports total cell allocations on one line and the bytes those cells cost on a second; cells alone cannot see a payload that stopped being copied, because a string holds its bytes inline and a copy and a window onto someone else's bytes are both exactly one cell. `PRISM_REUSE_STATS` reports how many cells the reuse pass rewrote in place. `PRISM_EFFOP_STATS` reports how many free-monad `EOp` cells were allocated, which the performance gate asserts is zero on the fusion corpus. `PRISM_DRIVE_STATS` reports native effect-driver activity.

### 14.9 Growable Arrays {#growable-arrays}

The growable `Array(a)` (see [the standard prelude](./spec.md#the-standard-prelude)) is an ordinary cell, `{ rc, tag 0, arity cap+1, len, elem0 .. }`, with the length word stored odd-tagged (low bit set, so the collector skips it as an immediate per [value representation](#value-representation)) and unused slots held at zero. Because it is a normal cell, reference counting recurses into its live elements with no special case. Every array operation borrows its array argument. `array_get` returns a counted element; `array_set`, `array_push`, and `array_pop` write in place when the array is uniquely owned (refcount 1) and copy otherwise, so functional array code runs as mutation exactly when ownership permits. `array_push` doubles the capacity when full, making appends amortized O(1). The prelude's `HashMap` is a separate-chaining hash table layered on this array, with an FNV-1a hash written in Prism (so iteration order is a deterministic function of the inserts); it is library code, not a runtime primitive. The byte buffer behind `Bytes` and the string builder follow the same rule.

Ownership is a property of the whole program, not of the loop that does the updating. An accumulator threaded through the free-monad tier's monadic bind is not uniquely owned where it is updated, so the in-place path does not fire and each step copies what has accumulated so far: an accumulation that is linear when its enclosing code fuses is quadratic one effect row away, and the two read identically in the source. Tier selection is a cost decision and never changes what a program computes (see [effect lowering](#effect-lowering)), so no oracle that compares outputs can see the difference and only an allocation counter can. The [performance gate](#lint-telemetry-and-parity) therefore pins the allocation slope of the readers that would otherwise regress in silence, measured in cell bytes rather than cells because a copy and an alias cost the same single cell.
...
Tier parity checks the ladder through the backends; a tier-equivalence gate checks it on Core directly. Every runnable corpus program is lowered five times, once with the cascade floored at each rung, and all five traces must equal the automatic position's, which is the only one free to take the pure and evidence rungs. Working on lowered Core rather than native binaries makes the sweep cheap enough to cover the whole corpus and then a generated one: a deterministic program generator points the same relation at fresh programs and greedily shrinks any divergence to a minimal reproducer. The gate also requires each adjacent pair of positions to move lowered Core somewhere in the corpus, so it cannot pass by two rungs having quietly collapsed into one.
...
### 15.5 The Assurance Matrix {#assurance-matrix}
...
| -------------------------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Rust phase types and typed-Core phase markers                                    | at compile time of the compiler itself       | illegal phase transitions and sugar-after-desugar are unrepresentable in the compiler's own types           | the Rust compiler; see [bootstrapping and self-hosting](#bootstrapping-and-self-hosting) | semantic correctness of any individual rewrite                                                                      |
...
The mid-level Core-to-Core tier is a composable pass framework in the spirit of GHC's `[CoreToDo]` pipeline. One shared traversal (`Rewrite`/`Visit`) replaces the hand-rolled Core walkers, so newtype erasure, dictionary and higher-order specialization, free-variable collection, call collection, and substitution all ride a single visitor. The canonical hasher from [architecture](#architecture) and the tail-recursion classifier from [reference counting and FBIP reuse](#reference-counting-and-fbip-reuse) remain bespoke because byte order and tail context are semantic, but both use explicit event worklists. Typed and raw read-only descents use explicit heap worklists with canonical enter/leave scope frames and logical-depth accounting; raw structural rewrites use the same discipline with postorder reconstruction frames. Context-neutral typed rewrites can likewise reconstruct terms from their type, signature, instantiation, forwarding, binder, and pattern hooks; witness substitution and whole-function effect-row discharge use that route. Type-witness internals and control-sensitive typed policies still use the recursive rewrite path behind the finite Core stack boundary. Free-variable collection is a scope policy on the raw visitor, while latent-effect, handler-shape, allocation-grade, linearity, and reuse-token analyses use dedicated postorder reduction stacks. Typed effect queries collect operation names through the shared visitor and borrowed thunk bodies through a lifetime-aware heap worklist. Each pass is a `CorePass` keyed by a `PassStage`, and the whole pipeline runs from one ordered, level-keyed list through a single `opt::run` entry.
...
A literal lambda argument qualifies through a lifting step that runs first. Each closed local lambda that provably reaches a collapsible application is hoisted out of its enclosing function into a fresh top-level definition, and the local binding becomes a closed eta-wrapper over the hoisted name, exactly the shape the specializer recognizes. The body moves once, so devirtualization works with a single named definition instead of duplicating the lambda into every clone. Every guard fails closed: a lambda that captures a local, a body that installs a handler or names any effect operation (its discharge is placed against the lexical region the binding sits in, which a hoist would leave), a binder that never reaches an application, or an enclosing function with type or row quantifiers (whose lambda could capture a rigid variable no top-level definition may mention) all decline, and a declined lambda stays exactly where it was, compiled as an ordinary closure.
...
### 16.5 ExactSize {#pass-exact-size}

- **Stage:** pre-lowering
- **Levels:** `-O1`, `-O2`
- **Disable:** `PRISM_NO_EXACT_SIZE` (or `no-exact-size = true` in configuration)

Exact-size construction recognizes the structural list-to-array builder chain used by `Array` and `Frozen` comprehensions. When the interprocedural summary proves that a call produces exactly a literal count or the value of an integer parameter, the pass redirects that call through bounded clones: the entry allocates the destination once at its final size, writes the head, and a fill loop writes the remaining indices. The original growable builders remain for every unproved call.

Recognition fails closed. The growable function must have the expected empty/cons case, pushes must thread one destination linearly, the tail call and wrapper chain must be unambiguous, and no extra computation may be hidden in the shape. An upper bound is not exact enough; filtering, effects, unknown calls, or an unfamiliar builder keep the growable path. The transformation changes allocation cost only, preserving element evaluation, failure order, and the value returned by every backend.

### 16.6 Simplify (Gentle Simplifier) {#pass-simplify}
...
### 16.7 Inline {#pass-inline}

- **Stage:** late
- **Levels:** `-O1`, `-O2`
- **Disable:** `--no-inline`

A bounded inliner pastes a non-recursive callee into a call site, alpha-renaming every binder so no name collides. A function with one call site is admitted directly. A function with several sites is admitted only when its summary proves a constant or unchanged-parameter result, zero allocation, no callback requirement, no closure construction, and an empty effect row, and its body is at most sixteen computation nodes. Multi-site splicing additionally requires an effect-row-neutral call. Anything larger, allocating, effectful, closure-building, or unknown remains a call; the `Simplify` that follows optimizes across an admitted boundary.
...
### 16.9 The Optimizer Fact Domain {#optimizer-fact-domain}

The passes above share one fact substrate rather than each keeping a private map of what it has learned. Its local environment records what a binder provably holds in the region where the entry is live, in exactly four shapes: a **constant** (a literal), a **must-alias** (the binder holds exactly another in-scope name's value), a **constructor** application with its field values, and a **product** (a tuple or unboxed product with its fields). The domain stores must-facts only. Nothing speculative enters it, a binder whose value does not classify simply has no entry, and a thunk deliberately never classifies, since remembering one invites inlining that duplicates work. Every simplifier rewrite that needs a value's shape (case-of-known-constructor, projection from a known product, copy-propagation, constant branch selection) queries this environment.

Two disciplines keep the domain sound. Classification looks through representation-only wrappers (a scalar reinterpretation, an erased newtype), but every rewrite carries the original wrapped value forward unchanged, so a wrapper is never lost to a fact that saw past it. And an entry dies with its scope: binding a name removes the entries it shadows and every entry whose stored value mentions a shadowed name, so no fact survives into a region where consuming it would capture the wrong binder. Iteration over the environment is deterministically ordered, so the rewrites it drives are a pure function of the program like everything else in the pipeline.

Control flow contributes refinement facts of its own. Inside a match arm the scrutinee is proven to be that arm's constructor; the refinement records the constructor's name and the arm's field binders rather than fabricating a value (only the verifier knows a constructor's runtime tag), and a nested match on the same scrutinee collapses by rebinding those binders. The same machinery selects constant branches and deletes arms the refinement proves unreachable, and those folds survive into the emitted code: a branch the facts decide at compile time never reaches the backend.

Above those scoped facts, a per-function summary over verified typed Core records the result shape, checked effect row, allocation bound, mutable-capture class, invoked callable-parameter slots, and exact, upper-bound, or unknown result cardinality. Direct-call strongly connected components are solved callee-first to a deterministic fixed point. Unknown or opaque structure joins conservatively. The inliner consumes the result, allocation, capture, callback, and effect fields; [`ExactSize`](#pass-exact-size) consumes cardinality. Neither is allowed to change evaluation order or failure behavior on the strength of a cost fact.

The substrate has a read-only surface. `prism dump optimizer-facts` prints a versioned fact sheet (`prism-optimizer-facts-v2`) over the optimized pre-lowering core: one name-sorted row per definition, carrying its computation-node size, direct and indirect call counts, sorted callees, the original explanatory tail-result fact, and a `summary` block containing the typed interprocedural fields. The explanatory result is honest about ignorance: where no shape is provable it prints `unknown:` followed by the first construct that blocks the analysis (a branch join, an effectful tail, a computed application), so each row names exactly what a stronger fact would have to see through.

Summary encoding is canonical and name-sorted rather than interner-order-dependent. With the compiler cache enabled, a cold dump stores that encoding under the frontend subject identity, analysis schema, compiler fingerprint, pre- and late-pass fingerprints, source-input digest, and build mode; a warm dump recomputes it and requires byte equality. There is deliberately no decoder yet, so this is a determinism and invalidation-closure gate rather than reused analysis. The dump decides nothing and cannot perturb a compile. A committed fixture corpus pins its schema and isolated optimizer costs, so resolving a fact moves an expected artifact or measured number deliberately, never silently.

### 16.10 Optimization Levels {#optimization-levels}
...
`-O1`, the default, is the real optimization level. On top of `EraseNewtypes` it runs [`Specialize`](#pass-specialize), [`HoSpecialize`](#pass-ho-specialize), and [`ExactSize`](#pass-exact-size) before effect lowering and, after it, the late pipeline [`Simplify`](#pass-simplify) -> [`Inline`](#pass-inline) -> [`Simplify`](#pass-simplify) -> [`Cse`](#pass-cse) -> [`Simplify`](#pass-simplify): dictionary and constant-callable specialization, exact-size destination selection where cardinality is proven, then a gentle simplifier brought to a fixed point around a bounded inliner and scalar CSE. This is the GHC simplify/inline/simplify shape, and it is what the compiler runs unless told otherwise.
...
```text
Fuse -> EraseNewtypes -> Specialize -> HoSpecialize -> ExactSize,
then Simplify -> Inline -> Simplify -> Inline -> Simplify -> Cse -> Simplify
```

### 16.11 Controlling the Pipeline {#explicit-pass-lists}
...
`--no-specialize` and `--no-ho-spec` are the flag forms of `PRISM_NO_SPECIALIZE` and `PRISM_NO_HO_SPEC`; each disables only its named pass. Exact-size construction currently exposes the environment/configuration switch documented in its pass section rather than a command-line subtraction flag. `--no-erase-newtypes` is honored but rarely wise, since both backends assume newtype erasure has run.
...
`<names>` is a comma-separated list in run order; a bare list with no marker is the pre stage. The pre passes are `Fuse`, `EraseNewtypes`, `Specialize`, `HoSpecialize`, and `ExactSize`; the late passes are `Simplify`, `Inline`, and `Cse`. Each section is exactly the passes named, with no level defaults filled in, so explicit means explicit. The `-O1` pipeline written out in full, and a pre-only run that stops after specialization:

```console
prism PROGRAM --passes 'pre:EraseNewtypes,Specialize,HoSpecialize,ExactSize;late:Simplify,Inline,Simplify,Cse,Simplify'
...
### 16.12 Controlling LLVM Codegen {#controlling-llvm-codegen}
...
### 16.13 Lint, Telemetry, and Parity {#lint-telemetry-and-parity}
...
| `PRISM_NO_EXACT_SIZE` | disables exact-size destination construction                                           |
...
| Flag | Name      | Default | Action                                                                           |
| ---- | --------- | ------- | -------------------------------------------------------------------------------- |
| `t`  | `types`   | on      | Show each inferred result type.                                                  |
| `e`  | `effects` | off     | Restore an explicit `! {}` on pure types.                                        |
| `s`  | `timing`  | off     | Report evaluation time.                                                          |
| `h`  | `holes`   | off     | Permit [typed holes](./spec.md#typed-holes) as deterministic interpreter faults. |

Empty-row elision is a REPL presentation policy shared by expression results, declaration echoes, and `:type`; non-empty or open rows remain explicit, and the canonical type printer is unchanged. Even with hole deferral enabled, `:type` reports a hole rather than executing it. A multi-line block runs between `:{` and `:}`, or is auto-detected when a line opens a layout block that remains unclosed.
...
Precisely, every hash below is a BLAKE3 digest of a token stream in which every field is self-delimiting: a name rides length-prefixed, no node opens with a digit, and any field ending in digits is closed by a terminator. Both halves are necessary because a length prefix is itself digits, so a numeric field left open would be read as part of the length of the name after it. Resolving one variable reference, inside the structural walk of a `Comp`/`Value` tree, is a four-way case split:

```text
tok(s)     = len(s) : s          a length-prefixed token
end(x)     = x ";"               closes a field that ends in digits

refer(s)   = "b" ++ end(i)       s bound at de Bruijn depth i (a param, let, or match binder)
           | "r" ++ end(idx(s))  s is a member of this definition's own recursive group (SCC)
           | "h" ++ end(H(s))    s is an external dependency, already hashed (Merkle substitution)
...
│  ├─ refs                      caller-named tag -> content hash
│  └─ lock                      advisory lock serializing the index writers
├─ queries/<kind>/<2 hex>/...   durable cache bindings, one subtree per query kind
├─ decisions/<kind>/<locator>   the query-fact ledger a rebuild is explained from
...
| `index/refs`         | `prism-store-refs<TAB>v1`      | `<tag><TAB><hash>`                                            |
| `meta/<sharded>`     | `prism-store-meta<TAB>v1`      | one `name` / `type` / `doc` key per line, `<key><TAB><value>` |
| `verified/<sharded>` | `prism-store-verified<TAB>v2`  | `<check-kind><TAB><scheme><TAB><identity><TAB><pass or fail>` |
...
The root `VERSION` carries the [hash-scheme](#content-addressed-core) tag and the store-format tag on their own lines, and a store whose either tag this build does not speak is refused outright rather than read under the wrong assumptions. Every hash-keyed layer shards git-style on the first byte of the hex digest (`<first two hex>/<rest>`) so no directory grows without bound: both blob layers, both hash-keyed metadata layers, and each query kind's own subtree.[^atomic-write] The four `index/` files, which read-modify-write a whole file, additionally serialize their writers through the advisory `index/lock`, best-effort because a lost index binding is recovered on the next commit.

[^atomic-write]: Every publication lands atomically: bytes go to a uniquely named `.tmp.*` file in the destination directory, then the commit point hard-links an immutable entry or renames a mutable one into place. An ordinary write flushes before publishing. A native compile with the compiler cache enabled instead gives its session an isolated transaction: workers share that transaction's pending queue and read their own staged objects, while unrelated store handles cannot observe or publish them. A successful compile pays one durability barrier per device before publishing the queue in staging order; dropping the final joined handle is a best-effort cache-only backstop. Read-modify-write indexes and verification records remain eager under their advisory lock, which spans both the read and publication. There is no process-global pending-write state.
...
### 21.2 Bounded Storage and Reclamation {#storage-reclamation}

Sharding bounds how wide a directory grows, not how large the store grows, and everything below `index/` is a cache whose loss is a correct future miss rather than a lost fact. Each high-churn layer therefore bounds itself at publish time. A shard carries a watermark pair on both dimensions that can run away, the entry count and the byte total, and a publish that leaves its own shard over either cap retires that one directory's oldest entries until the overflowing dimension sits at its low mark. Because keys are hashes, shards fill uniformly, so bounding every shard bounds the layer; retirement is paid continuously and locally instead of deferred to a whole-store crawl that grows more expensive the longer it is postponed. Eviction never consults liveness, because it does not need to: age is the only signal, a hit refreshes the entry's age so a republished entry is not retired for being old, and an evicted entry reads as an ordinary miss on the next lookup. One publish also never retires more than a bounded batch, so a shard inherited far above its budget is ground down across many publishes rather than stalling one.

| Layer               | Per-shard budget       | Resulting layer ceiling      |
| ------------------- | ---------------------- | ---------------------------- |
| `objects/`, `meta/` | 1024 entries or 16 MiB | about 256K entries and 4 GiB |
| `queries/<kind>`    | 256 entries or 1 MiB   | about 64K bindings per kind  |

Publish-time bounding is the continuous defense; `prism store gc` is the deep clean, and it collects only what the store can actually prove unreferenced. The mark set is the outputs still bound by surviving query entries plus every hash the name, dependency, canonical, and ref indexes point at, and the sweep removes an object or metadata blob only when it is both absent from that set and older than the age cutoff (`--days`, thirty by default). The cutoff is the safety margin for the store's one blind spot and for one race: content addressed by something outside the store, a lock pin in another project for instance, is invisible to the mark phase, and a commit that has not yet reached its query or index binding must not be swept out from under itself. A caller holding content the store cannot see records a ref for it, which makes the reference visible and the object permanent until the ref moves. The sweep fans its per-shard scans out across a bounded worker pool and beats progress once per finished shard directory, a census of every layer prints before it and after it, and `--dry-run` computes the same figures without touching a file.

A layer that predates publish-time bounding, or one whose eviction path stopped firing, can hold millions of files, and walking such a tree in place would grind for hours before reclaiming anything. A layer or query kind whose census sits far past its eviction ceiling is therefore retired wholesale rather than swept: a manifest recording the tree's origin is written inside it, then one rename moves it to a dot-prefixed sibling at the store root. Readers only ever open exact live paths, so the store is healthy the instant the rename lands and the visible cost is that single rename. The retired tree then drains offline, salvaging by hard link the entries the live set still references or that were written recently enough to be an in-flight commit, refreshing each salvaged entry's age so the next eviction sees it as current, and removing the rest. A crash mid-drain leaves the tree in place; the next sweep finds it by prefix, reads the manifest, and resumes.[^retire-data] Because a broken eviction path is what puts a layer in that state, publishes also carry a tripwire: one publish in 256 lands in a sample shard, counts that one directory, and past a threshold names the layer-wide estimate once per process, long before the layer grows into a disk-eating catalog.

[^retire-data]: The origin travels as data inside the tree rather than encoded in the directory name, which is a case of the same rule the compiler's generated identifiers follow: a fact a later phase needs is carried on the thing itself, never parsed back out of a name. The query layer keeps its own layout stamp for the same reason, so a layout change there can never refuse the objects and certificates beside it; a tree predating the stamp is not opened through the current paths at all, reads as ordinary misses, and has its relics removed rather than migrated.

### 21.3 Verification Caching {#verification-caching}

A stored object carries its verification verdicts alongside it. A check that a definition passed, its interpreter [parity](#lint-telemetry-and-parity), its doctests, or its expect tests, is recorded as an append-only verified record keyed by the definition's content hash and carrying both the hash scheme and the full behavior-affecting artifact identity. A later build reads a matching verdict rather than re-running the check, so an unchanged definition does not re-run its tests, doctests, or parity comparison, and the total cost of verifying a change tracks the Merkle closure of that change rather than the size of the program. A scheme bump invalidates every prior verdict at once; a toolchain, target, backend, or flag change moves the artifact identity. Neither can silently reuse a stale pass. Because the content hash is invariant under formatting and renaming, those edits keep otherwise matching verdicts intact.
...
### 21.4 Incremental Compilation as a Query {#incremental-compilation}
...
Because worker count is not a semantic input, sequential and parallel schedules (`PRISM_QUERY_THREADS`) may visit ready queries in different orders but must converge on byte-identical artifacts. Results and worker failures are folded in input order; a worker panic becomes a structured internal compiler error after every scoped worker has been joined, never an unwind through the embedding caller. The query oracles compare dependency-layer checking and decisions across worker counts, then compare fresh, incremental, sequential, and parallel stores, including binaries, canonical observations, query bindings, object identities, and normalized whole-program/SCC LLVM structure. The scheduler is thereby held to the language's determinism contract from inside the compiler, as parity holds generated programs to it from outside.
...
### 21.5 The Kont Envelope {#the-kont-envelope}
...
scheme tag     length-prefixed "prism-core-hash-v2"; a foreign scheme is rejected first
...
One canonical uvarint tag table (`TAGS`) spans all six domains, so encode and decode cannot drift. An index is untyped on the wire, and the builder validates each referent's tag against the domain it is used in, so a cross-domain reference in a hostile frame is rejected rather than misread.[^suspend-depth]
...
### 21.6 Source, Surface, and Core Identity {#three-identities}
...
Distribution is therefore the content-addressed store carried across a network. A package's `prism.toml` carries its required name, version, authors, maintainers, and validated SPDX license identifier, plus optional descriptions, project URLs, and changelog, license, and readme file lists. This is catalog metadata, not build identity: the content hash remains authoritative. `prism check --licenses` reduces the transitive path-dependency graph to a sorted `package version - license` audit.

A project declares its dependencies in a `[dependencies]` table in the same manifest, in one of three forms: a `path` to a local directory, a `git` URL paired with an opaque tag, or a bare content-hash pin naming an exact definition graph. The three are the same `DepSource` the resolver consumes, differing only in how a name is turned into a root hash. Edits to the table go through a format-preserving manifest writer that rewrites only the dependency lines and leaves every comment, blank line, and untouched byte exactly where the author put it, so `prism pkg add` does not reformat a hand-maintained manifest.
...
## 25. Open Compiler {#open-compiler}

Prism is becoming an **open compiler**: its syntax machinery is an ordinary, versioned Prism library rather than a facility available only behind the compiler executable.[^open-compiler] The `Syntax.*` modules expose source files and byte spans, tokens and trivia, an ordered semantic surface AST, structured diagnostics, parser entry points, resolved bodies, generic traversal, and byte-idempotent codecs for those artifacts. A Prism program can decode, inspect, parse, traverse, and re-emit syntax values under the same explicit-input determinism contract as the rest of the language. The Rust/LALRPOP parser remains the production authority while the Prism parser and its diagnostics complete parity.
...
The front end bootstraps through the same opening. A Prism-written lexer, interpolation scanner, layout pass, and parser families run beside the Rust/LALRPOP implementation and are diffed against its versioned artifacts across accepted, negative, generated, and hostile corpora. The self-parse harness exercises the Prism source itself, while Rust/LALRPOP remains the comparison implementation until the full front end and its diagnostics converge. The compiler selects the implementation from its pinned toolchain and standard-library root, so a project cannot change the language it is compiled with by shadowing a `Syntax` module.
...
This provides white-box reuse: tools can use the real lexer, parser, representations, and printers, then add modular analyses and transformations without forking the compiler. Macros, quasiquotation, user-defined syntax, and arbitrary compilation hooks remain deliberately absent. A fully open Prism-in-Prism compiler is the long horizon: rebuilding the floor while standing on it is hard enough, and also extremely cool.

[^open-compiler]: The term comes from John Lamping, Gregor Kiczales, Luis H. Rodriguez Jr., and Erik Ruf, “An Architecture for an Open Compiler,” _Proceedings of the IMSA '92 Workshop on Reflection and Meta-level Architectures_, 1992, pp. 95–106.

## 26. Bootstrapping and Self-Hosting {#bootstrapping-and-self-hosting}

The compiler is currently written in Rust. The bootstrap path is conventional: Rust is stage 0, it builds the Prism compiler sources as stage 1, and stage 1 builds stage 2. Agreement between stages is the practical fixed-point check.
...
`prism bootstrap check FILE` runs the T1 checker from the `tc` package. Rust first accepts the program and exports `tc-input`, resolved syntax, surface syntax, and checker facts. Prism decodes those artifacts and independently infers the supported declaration subset, including polymorphic effect rows and handlers. Handler type arguments are lexical evidence: anonymous bodies and named private instances share one instantiation with their clauses, while clause bodies remain in the surrounding scope. `Data.Diff` locates the first fact that differs.

Batch checks prepare and verify the checker once from isolated tool-package roots, then run each target with fresh evaluator and effect state. Project modules therefore cannot shadow the checker, and no target facts or mutable machine state leak into the next comparison.

The report separates parity from coverage. Unsupported syntax reduces the covered node count. It does not create a disagreement. `--json` emits `prism-bootstrap-check-v2`; the text view shows the same status, coverage, and first divergence. Rust remains authoritative in both modes. Agreement is string equality of a versioned canonical scheme spelling (`prism-scheme-canon-v1`): both checkers alpha-normalize leading `forall` binders to `$0 $1 ...`, the report stamps the contract identifier in its `scheme_contract` field, and the shadow's protocol header carries the same identifier so a drifted normalization refuses loudly instead of quietly changing what agreement means.

Two parsers exist today and that is not a permanent arrangement, so the terms under which one of them stops being maintained are written down before the question is live rather than after. Should parser authority move to the Prism implementation, the Rust/LALRPOP parser becomes a diverse comparison oracle for exactly one release and nothing else. Operationally that means the differential and self-parse gates keep running it, a grammar change lands in the Prism parser first and is mirrored into the oracle only so far as keeping the diff meaningful requires, and a divergence between the two is judged with the Prism parser as the authority and the oracle as the second opinion. The release after the transfer either deletes the oracle or records an explicit decision to keep paying for it, with the cost named. The one outcome ruled out in advance is drifting into maintaining two changing grammars because nobody chose, which is the failure this paragraph exists to prevent. A rollback path to the Rust parser stays available during the transition, and it is explicit and visible in the gates rather than a silent production fallback, because a fallback nobody can see is indistinguishable from the transfer not having happened.
...
The surface is nineteen top-level commands plus five noun groups (`exec`, `lineage`, `patch`, `pkg`, `store`), each group collecting the verbs that share a subject.
...
| `prism lint [paths..]`               | Judge source files against the house style rules.                   |
...
| `prism store gc [--days N] [--dry-run]`   | Reclaim unreferenced cache entries.       |
...
| `optimizer-facts`       | The versioned [optimizer fact sheet](#optimizer-fact-domain): per-definition sizes, call counts, callees, and result facts. |
| `core-json`             | The core as a JSON tree the Lean model reads (the [differential oracle](#the-model-as-a-differential-oracle)).              |
| `core-identity`         | The [identity surface](#the-identity-surface): the exact pre-optimizer core, metadata, groups, and dependency hashes.       |
...
### 27.4 Environment Variables {#environment-variables}
...
| `PRISM_QUERY_THREADS`        | Positive worker count for independent compiler queries (default: host parallelism, capped at 8); collection and artifacts remain deterministic.                                                                                                                              |
...
| `PRISM_TOOL_PACKAGES_ROOT`   | Load the compiler-owned Prism tool packages from this `packages/` directory while developing them; installs use the sources embedded in the binary. A developer input, never read from the checked project, so a project cannot choose the checker or linter that judges it. |
...
| `PRISM_WARN_PRELUDE_CAPTURE` | Prelude name capture severity: `warn` (default), `strict`, or `off`.                                                                                                                                                                                                         |
...
| `PRISM_ALLOC_STATS`   | Report the total number of runtime cell allocations, and the bytes those cells cost.                             |
...
| `PRISM_PROMOTE_STATS` | Report what escaping a region cost: cells promoted, edges that reused a copy, and the promotion walk's own size. |
...
### 27.5 REPL Commands {#repl-commands}
...
`prism index` writes a deterministic JSON view of a codebase for browsers, review tools, and agents. It indexes every listed module, not only definitions reachable from the program entry point. The result is a projection of facts the compiler already computes, so it shares names, types, effects, dependencies, documentation, and identity with the build rather than running a separate analysis.

Each definition records its canonical name, kind, source, documentation, signature, effect row, claims, and the content address appropriate to that kind. Constructors, effect operations, and class methods are recorded as searchable members of their owning declarations. The artifact also contains the relationships needed for navigation: `calls`, `uses-type`, `performs`, `handles`, `instance-of`, and `tests`. Edges may point outside the indexed unit, and the test layer records whether tests were included, absent, or unavailable because test elaboration failed.

Definitions are addressed rather than identified by file and line. Source ranges remain useful for rendering, but content addresses let tools align revisions across formatting, file moves, and local renames. This is the main architectural goal of the index: review and navigation state should follow the code's identity instead of being discarded whenever its location changes.

Identical inputs produce byte-identical artifacts. `prism index --check` can therefore gate a committed index in CI. Source text is embedded by default so a consumer does not need the working tree; `--no-source` omits it when the consumer already has the files.
...
The [code viewer](https://sdiehl.github.io/prism/viewer/) is the first consumer. It searches modules, definitions, declaration members, and source text, then opens definitions as a working set of cards instead of replacing the current view. This follows the working-set approach explored by [Bragdon et al. (2010)](bibliography.md#bragdon-2010).

A card shows the definition's signature, documentation, source, address, claims, effect row, tests, and relations in both directions. Compiler-produced occurrence rows make names in the body navigable, and type spans provide hover types for parameters, locals, and pattern binders. Collecting those hover types instruments the checker, so the index takes them on a separate pass and grafts them onto a definition checked the ordinary way: the addresses an index publishes are exactly the ones [`dump stdlib-hash`](#dump-phases) reports, because a fact that exists to be displayed is never allowed to reach a hash. Signatures and bodies use syntax-highlighting spans produced by Prism's lexer. Compiler primitives and references outside the artifact are labelled rather than presented as broken links.

The viewer loads an index directly, including one supplied with `?src=<url>`, and does not run the compiler or load the WebAssembly build. This keeps it useful for private or independently generated indexes as well as the published standard-library index.

### 30.2 Revisions {#index-revisions}

`prism index --diff` compares two artifacts using both source and content identity. It classifies definitions as unchanged, cosmetic, changed, moved, or in the dependency cone. The cone category separates definitions whose own source is unchanged but whose address moved because a dependency changed. An unambiguous rename is reported as a move rather than as a deletion and addition.
...
The viewer stores read marks and notes locally against definition addresses. When the code changes, it can distinguish a reformat from an authored edit or a dependency change and ask for attention only when the recorded identity warrants it.

The index can also assemble a context packet for a selected set of definitions, including source, types, effects, callers, tests, and a budgeted transitive call closure. There is no question-answering interface for packets today. The longer-term goal is to let review tools and agents ask questions over explicit compiler facts without turning the index into another compiler database or embedding model-specific behavior in the artifact.

### 30.4 Occurrences {#occurrences}

`prism dump occurrences` exports the source ranges of resolved names with their canonical targets. Read forward, the rows provide go-to-definition; grouped by target, they provide find-references. They are collected by the renamer that decides what each name means, so shadowed locals are excluded and unqualified imports resolve to canonical names.

The index combines these rows with type spans and declaration-member data to make rendered code navigable. The export remains intentionally narrower than a full syntax tree: it records the name positions the compiler can identify precisely and leaves broader source constructs to the ordinary syntax artifacts.
