...
| `prism run PROGRAM`                  | Type-check and run in the interpreter, with real stdin/stdout (`exit(n)` becomes a real process exit); `--defer-holes` turns reached [typed holes](./spec.md#typed-holes) into deterministic faults; `--record PATH` writes a `.replay` trace, `--lineage PATH` a run sidecar, and `--durable PATH` resumes a crash-safe log. |
