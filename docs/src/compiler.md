...
13. **Nothing is ambient; every dependency is data.** The world a program touches is spelled in its row; the moments it observes are frames in its trace; the identity of a definition is the hash of its behavior; the origin of every artifact is an edge in its lineage. What is not declared cannot happen within the guarantee; what happened can be replayed; what exists can be named; what was produced can be traced.[^p-no-ambient]
...
### 21.5 Source, Surface, and Core Identity {#three-identities}
...
A card shows the definition's signature, documentation, source, address, claims, effect row, tests, and relations in both directions. Compiler-produced occurrence rows make names in the body navigable, and type spans provide hover types for parameters, locals, and pattern binders. Signatures and bodies use syntax-highlighting spans produced by Prism's lexer. Compiler primitives and references outside the artifact are labelled rather than presented as broken links.
