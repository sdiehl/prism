...
| [Effect lower](#effect-lowering)                      | remove handlers and operations                                                                                                | `src/core/typed/effect_lower/`             |
| [Reference count](#reference-counting-and-fbip-reuse) | insert `dup`/`drop`, then reuse                                                                                               | `src/core/fbip/`                           |
...
| `prism lineage why-recompiled` | explain reuse and recompilation across the durable query graph              |
...
| ------------------ | ------------------------------------------------------------- | -------------------------------------------------------------- |
| Surface tree       | the program as written, sugar and unresolved names and all    | `Expr<Surface>` (`src/syntax/ast.rs`)                          |
...
| Core               | value/computation split; every effect and allocation explicit | call-by-push-value in administrative-normal form (`src/core/`) |
| Typed core         | witness-carrying types and effects across the verified prefix | `src/core/typed/`                                              |
...
The surface tree is parameterized by its compilation **phase** (`src/syntax/ast.rs`). An `Expr<P>` holds its sugar-only forms, its parse-time markers, and its surface-only handler clauses in fields whose types are associated types of the phase `P`: in the `Surface` phase those are the real sugar payloads, and in the `Core` phase, desugar's output, they are the uninhabited type `Never`. Because `Never` has no values, a sugar node cannot be constructed in the core phase at all, so a missed desugaring is a type error in the compiler rather than a runtime `unreachable!`, and every later pass over `Expr<Core>` is statically excused from matching the sugar cases.
...
Before the tier cascade, `src/core/typed/effect_lower/arena.rs` finds functions that install an `Alloc` handler and resolves the entry thunks passed to them. It closes over the direct call graph and computes `arena_only = arena_reachable \ otherwise_reachable`. In that arena-only set, a returned constructor or tuple is split into `Do(alloc, arity)` followed by `InitAt(cell, value)`. The handler discharges `alloc` to `prim_arena_bump`; after effect lowering has removed the `Do`, the shared emitter lowers `InitAt` by storing the tag and fields into the returned raw cell. This transformation runs before tier selection, so every effect-lowering strategy sees the same allocation boundary.

The exclusion is part of the contract. A helper reachable from both arena and non-arena paths stays on the ordinary allocator unless the compiler can specialize it, preserving byte identity for its non-arena callers. Closures and `RefNew` cells also remain on the ordinary path. [`examples/arena.pr`](https://github.com/sdiehl/prism/blob/main/examples/arena.pr) exercises an arena-only list builder; [`examples/arena_shared.pr`](https://github.com/sdiehl/prism/blob/main/examples/arena_shared.pr) and `tests/compiler/arena.rs` pin the shared-helper exclusion. `@ noalloc` still rejects `Do(alloc)`: changing the allocator makes a cell cheaper, not nonexistent.

The live C entry point `prism_bump` delegates to `prism_alloc`, so an `InitAt` cell has ordinary reference-counted ownership. `runtime/prism_arena.c` implements and self-tests a block-chained allocator, but the canonical runtime source manifest excludes it. The linked runtime has no arena-owned tag bit, handler-scoped reset/destroy, or zero-per-iteration-allocation guarantee. Consequently `with_arena` is semantically active and parity-checked, but it provides neither bulk reclamation nor arena performance in this configuration.
...
Detection is the wrong instrument. Rejecting `fn bump` is a language regression, and a check against the runtime's current symbol table rots the moment the runtime gains a function. So the prefixes are chosen instead so that collision is not a thing that can be spelled: each is `prism` followed by a distinct character, so two symbols from different definers disagree at a fixed index and cannot be equal, whatever either side is renamed to later.[^symbol-namespaces] The argument is machine-checked twice over. That the prefixes are pairwise distinct is a `const` assertion in `src/codegen/mod.rs`, so a change that collapsed two of them fails while the compiler itself compiles; that the runtime respects its half is a test over the embedded runtime sources (`src/codegen/rt.rs`), since the C side is where a stray definition would otherwise slip in unnoticed. One crossing is deliberate and is the reason the entry point is named at all: `main` is an ordinary Prism function, so the runtime calls it by its mangled name like any other, and that single name is the only place C reaches into the Core-function namespace.
...
The native runtime exposes `prism_bump(n)` for the compiler's [arena-allocation](#arena-allocation) path, but it calls `prism_alloc(n)`. The `alloc`/`InitAt` lowering is therefore representation-identical to ordinary construction. The separate `runtime/prism_arena.c` contains a block-chained bump allocator with create, aligned allocate, reset, destroy, and usage operations; CI compiles and runs its standalone self-test, but `build.rs` deliberately excludes it from the runtime manifest.

The linked runtime defines no handler-scoped region lifetime or arena ownership bit, and handler exit performs no region reset or destroy. Arena programs therefore use ordinary per-cell allocation and reclamation. Any region-backed implementation must make reference-count operations inert for arena cells and enforce the escape policy before reclaiming a handler's region.
...
Compiler CI combines differential testing, sanitizer passes, structural checks, and a mechanized Core model. The parity harness (`tests/native/parity.rs`) uses the interpreter as its reference: it runs every example on the interpreter and each native backend and asserts byte-identical output, and with `PRISM_CHECK_LEAKS` set, zero leaked cells.

What the parity harness and the tier-parity check actually diff is not raw stdout but a single typed artifact, the canonical observation trace (`ObservationTrace`, `src/provenance.rs`). A trace is an ordered sequence of `Observation` values, one entry per externally visible event a run produces: `Stdout`/`Stderr` chunks, `Capability` events (each a `CapEvent` recording a canonical operation label, its arguments, and its result, covering environment, console, filesystem, clock, and random reads), `FileCommit` records naming a written path by content digest, the terminal `Exit` code or `Return` value, and, when execution goes wrong, a `Fault`. The whole sequence folds to one `sha256` digest (`ObservationTrace::new`, `observation_digest`) that names the run's complete behavior, so two runs "agreeing" means their traces are equal, not that someone diffed stdout by hand. The interpreter and replay build this trace directly from the observations the evaluator records as it runs (`src/eval/mod.rs`); a native binary, which exposes only a process boundary, is captured through `ObservationTrace::from_process` (`src/provenance.rs`), which folds its stdout, stderr, and exit code into the same event vocabulary, and `process_projection` derives that same projection from a full trace so an interpreter run can be compared against a native one on equal terms. This is the one artifact both the parity harness (interpreter versus each native backend, `tests/support/mod.rs`) and the tier-parity gate (native binaries built under different forced effect-lowering tiers, `tests/native/tier_parity.rs`) diff to decide pass or fail, and it is the same trace a run-lineage sidecar hashes to prove a replay reproduced its recording exactly (`src/lineage/run.rs`, `src/cli/lineage.rs`): one comparison, one type, reused across the oracle, the backends, every tier, and replay, rather than an ad-hoc convention re-implemented at each site.
...
Because worker count is not a semantic input, sequential and parallel schedules (`PRISM_QUERY_THREADS`) may visit ready queries in different orders but must converge on byte-identical artifacts. `tests/compiler/module_queries.rs` compares dependency-layer checking and decisions across worker counts; `tests/native/compiler_cache.rs` compares fresh, incremental, sequential, and parallel stores, including binaries, canonical observations, query bindings, object identities, and normalized whole-program/SCC LLVM structure. The scheduler is thereby held to the language's determinism contract from inside the compiler, as parity holds generated programs to it from outside.
...
The surface is eleven top-level commands plus five noun groups (`exec`, `lineage`, `patch`, `pkg`, `store`), each group collecting the verbs that share a subject.
...
| `prism run <file.pr>`                    | Type-check and run in the interpreter, with real stdin/stdout (`exit(n)` becomes a real process exit); `--defer-holes` turns reached [typed holes](./spec.md#typed-holes) into deterministic faults; `--record PATH` writes a `.replay` trace, `--lineage PATH` a run sidecar.                      |
...
| `prism lineage why-recompiled [file.pr]` | Run the ordinary compiler queries and explain reuse or recompilation across module, optimizer, effect, backend SCC, closure-plan, object, and link facts.                                                                                                                                           |
...
```console
$ prism patch fetch app.pr increment > fetched.json
$ prism patch impact app.pr prism-core-v1:8cf0... > impact.json
$ prism patch create app.pr prism-core-v1:8cf0... replacement.pr > patch.json
$ prism patch submit app.pr patch.json > judgment.json
$ prism patch behavior app.pr patch.json corpus.json > behavior.json
$ prism patch commit app.pr
```
...
`prism patch serve <file.pr>` accepts one `prism-patch-protocol-v1` request per line. Its `fetch`, `impact`, `create`, `submit`, `behavior`, `commit`, and `discard` verbs use the same request payloads and return the same response payloads as the corresponding CLI commands. The reference client in `examples/patch_client.rs` exercises this interface through a child process; no editor or MCP transport is required.
...
| `--defer-holes`               | single-file `run`    | off                            | Permit typed holes through the interpreter frontend; reaching one emits a canonical fault. It cannot be combined with `--examples` or `--record`; native and WebAssembly compilation still refuse holes. |
