...
Its concurrency and mobility owe a separate debt, to Erlang and the BEAM: isolated fibers, failure treated as an ordinary value, and computation that can move between nodes. The divergence is in what travels. Where the BEAM copies messages between processes and trusts a peer by a shared node cookie, Prism moves the running computation itself as a content-addressed envelope authorized by hash ([suspend and resume](#suspend-and-resume)); the good ideas are inherited, the trust model and the unit that crosses the wire are not.
...
This mobility takes its good ideas from Erlang and the BEAM, then parts ways on the unit that moves. Erlang's isolated processes, its treatment of failure as an ordinary value, and its bias toward supervision over shared mutable state are all present here, carried by effect handlers and [structured concurrency](#structured-concurrency) rather than by a bytecode VM. What differs is what crosses the wire. The BEAM distributes by spawning processes and copying immutable messages between them; a running computation is never lifted off its stack and set down elsewhere, code is shared by hot-loading named module versions, and a peer is trusted by a shared node cookie. Prism moves the computation itself: `suspend` captures the live delimited continuation as a content-addressed envelope and `resume` reconstructs it against code it re-derives and proves identical by hash, so what travels is the suspended program's own state and what authorizes it is the bundle digest, not a name, a module version, or a cluster secret. It is process migration by code identity rather than message passing by location, and it inherits determinism and replay from the same content-addressed core, a guarantee distributed Erlang does not make.
