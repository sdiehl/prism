...
- Indexing `a[i]` / `a[i] := v` / `a[i] += e` over `Array` (Int key), `HashMap` (String key), read-only `String` byte, and `List`, dispatched at elaboration on the receiver's head type, so no new class. A read of a missing index performs `Fail` (`a[i] : Elem ! {Fail}`), composing with `??`/`?.`/`default` rather than an `Option` unwrap; writes are FBIP-in-place when uniquely owned, and nested `grid[i][j] := v` composes.
- Explicit named-instance selection moves from `f[name]` to `f(args, using name)`, freeing `[]` for indexing and converging explicit selection with first-class dictionary passing on one form; `using` is now reserved.
- Imperative control flow: `while`/`loop` lower to a tail-recursive prelude driver (constant stack, no per-iteration allocation), an unconditional `loop` to the bottom-typed `forever`; `break`/`continue`/`return` compile to non-resumable performs of internal, fully-handled effects, so none appears in a function's surfaced row, and a pay-as-you-go loop installs a handler only for the keyword it uses. The prelude's prior `while` is renamed `repeat_while`.
- A power operator `^` (right-associative, tighter than `*`): the method of a new `Pow` class, so `2 ^ 10` is bignum-correct `Int` and `2.0 ^ 10.0` is `Float`, with mixed `Int ^ Float` a type error. The prior integer `pow` is now `int_pow`.
- Two warnings against the user's own source (the prelude is exempt): an unused local binding and a name shadowing one in scope (a leading `_` and a consuming rebind `let s = f(s)` are exempt). Type annotations are now name-checked uniformly across parameters, returns, constraints, and rows: naming an undeclared effect or constructor is a hard error, and an annotation broader than the inferred row warns.
- `Set` gains `set_union`/`set_intersection`/`set_difference`; a project may replace the built-in prelude via `[package] prelude` in `prism.toml`.
- CLI: a bare `prism program.pr` compiles a single file to a native binary named after the source (`-o` to override, `--mlir` for the backend); `prism build` is now the project verb (nearest `prism.toml`), and interpreting is explicit via `prism run`.
- Local monadification confines the free-monad form to the connected component entangled with an escaping effect, keeping unrelated functions on their fused evidence/state path (a `perf_gate` case pins the pipeline at zero `EOp` cells) and falling back to whole-program lowering only when the split is unclean. Falling onto the fallback now warns on stderr by default (silence with `PRISM_QUIET`), naming the functions that lost fusion and the cause.
- Constant-stack effect handlers: a closed free-monad handler is driven by one self-tail-recursive `{n}@region` loop, so a tail-resumptive or function-answer state handler resumes via `musttail` and runs in constant native stack (a `perf_gate` case takes a `State` loop from ~10^5 to millions of iterations in a 2 MB stack). On by default, opt out with `PRISM_NATIVE_EFFECTS=0`; it changes the driver, not the reification (one `EOp` cell per operation), and ineligible handlers keep the previous driver.
- Compiler-internal correctness, with no change to accepted programs or emitted output: existential scope is enforced unconditionally at `solve`; numeric defaulting is deferred to one pass at generalization, so a later `i64`-fixing use picks the fixed-width lane; the free-monad driver templates are closed by construction; and FBIP reuse is a scoped `WithReuse`/`Reuse` core node, making free-once and spend-at-one-allocation well-formedness properties of the term (tracked by the Lean determinism proof).
- The Lean core model (`models/`) grows from a substitution small-step into a mechanized CEK machine proved to implement a big-step semantics, with unique normal forms, a progress trichotomy, and effect progress/unhandled lemmas, all `sorry`-free over `propext`/`Quot.sound`. It runs as a third differential oracle alongside the interpreter and native backends via a new `prism dump core-json` phase, checked against the interpreter on the compiler's real core across a fixture corpus.
- Hardening: the bignum allocator is overflow-checked like every cell, and each builtin's calling convention is derived from an exhaustive `Builtin` method, so a new builtin cannot ship without declaring its ABI. The reference grammar is single-sourced in `models/grammar.ebnf`, sliced into the specification by mdbook anchors.
