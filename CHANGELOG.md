...
- Syntax highlighting: definition bodies, and the rendered type and effect row, are highlighted and linked from spans the compiler's own lexer produced, baked into the index rather than computed in the browser — so the colouring cannot disagree with the compiler, and it is there at first paint.
- Index references: a constructor, effect operation, or class method links to the declaration it is written inside, and the artifact carries the compiler's primitive names so a reference to one reads as a primitive rather than as missing. Nothing in a whole-program index is left unexplained.
- Index members: a declaration records the constructors, methods, and operations it introduces, so a member is findable by name and its users are reachable from it — including an effect operation that programs perform and the library only handles.
- Index `uses-type`: walks the checked type structurally and covers the types written into a declaration's own signature, not only terms. Keying on the resolved symbol also removes false edges: a definition mentioning `Outcome` was previously linked to every same-named type in scope.
- Index effects: added `handles`, the complement of `performs` — handling an effect is what removes it from a row, so the definition that gives an effect its meaning is invisible to `performs` by construction. An instance also reports what its methods call and perform, which the lowering to top-level functions had hidden.
- Review state: the viewer remembers what you have read and the address you read it at, so a later visit distinguishes a reformat from a dependency shift from a real edit, and only the last two ask for attention. Notes attach the same way.
