...
- validates 65 typed Type/Pattern action rows against exact production, alternative, RHS, action, checked-action, selector, target, completion, and hook identities;
...
This generator completes structural Type authority, not whole-parser authority. Production Pattern parsing is also behind a generated facade, but closing its artifact ownership and freshness checks here remains unfinished. Pratt expressions and declaration-specific return binding remain separate generated families or escaped semantic seams.
