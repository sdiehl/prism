...
mod pat;
use decl::{
    fmt_class, fmt_data, fmt_effect, fmt_fn, fmt_import, fmt_instance, fmt_labels, fmt_param,
    fmt_pattern_decl, fmt_ty,
};
...
// The source being formatted, kept for the verbatim fallback: a formatter must
...
fn verbatim(start: usize, end: usize) -> String {
    SOURCE.with(|s| s.borrow().get(start..end).unwrap_or_default().to_string())
}
...
fn lead_comments(lo: usize, hi: usize, indent: usize) -> String {
    if lo >= hi {
        return String::new();
    }
    ...
    let mut gap = false;
    TRIVIA.with(|t| {
        for ev in t.borrow().between(lo, hi) {
            match &ev.trivia {
                Trivia::Comment {
...
// Whether any line comment sits in `[lo, hi)`. The inline fast paths check this
// before collapsing a node onto one line: a node carrying comments must take the
// laid-out path so `lead_comments` has somewhere to place them.
fn has_comments(lo: usize, hi: usize) -> bool {
    lo < hi
        && TRIVIA.with(|t| {
            t.borrow().between(lo, hi).any(|e| {
                matches!(
                    &e.trivia,
                    Trivia::Comment {
                        kind: BuiltinKind::Line,
...
pub fn format(src: &str) -> Result<String, Error> {
    ...
    TRIVIA.with(|t| *t.borrow_mut() = trivia);
    let out = fmt_program(&program);
    SOURCE.with(|s| s.borrow_mut().clear());
    TRIVIA.with(|t| *t.borrow_mut() = TriviaTable::new());
    Ok(out)
}
...
fn emit_leading_trivia(lo: usize, hi: usize, out: &mut String) {
    TRIVIA.with(|t| {
        for ev in t.borrow().between(lo, hi) {
            match &ev.trivia {
                Trivia::Comment {
...
fn fmt_program(prog: &Program) -> String {
    ...
    for i in &prog.instances {
        items.push((i.span.start, i.span.end, fmt_instance(i)));
    }
    for p in &prog.patterns {
        items.push((p.span.start, p.span.end, pubd(&p.name, fmt_pattern_decl(p))));
    }
    for f in &prog.fns {
        items.push((
            f.span.start,
            f.span.end,
            pubd(&f.name, fmt_fn(f, Mode::Layout)),
        ));
    }
    ...
    TRIVIA.with(|t| {
        for ev in t.borrow().after(prev_end) {
            match &ev.trivia {
                Trivia::Comment {
                    kind: BuiltinKind::Line,
                    text,
...
fn fmt_dot_recv(recv: &S<Expr>, indent: usize) -> String {
    let s = fmt_expr(recv, indent, Mode::Flat);
...
fn fmt_call_flat(f: &S<Expr>, args: &[S<Expr>], indent: usize) -> String {
    if let Some(s) = fmt_interp(f, args) {
        return s;
    }
    if let Some(recv) = try_recv(f, args) {
        return format!("{}?", fmt_dot_recv(recv, indent));
    }
    if let Some((name, recv, rest)) = dot_parts(f, args) {
        let rest_s: Vec<String> = rest
            .iter()
            .map(|a| fmt_expr(a, indent, Mode::Flat))
            .collect();
        return format!(
            "{}.{name}({})",
            fmt_dot_recv(recv, indent),
            rest_s.join(", ")
        );
    }
    let f_s = fmt_expr(f, indent, Mode::Flat);
...
fn fmt_block(e: &S<Expr>, indent: usize, from: usize) -> String {
    ...
    while let Some(step) = block_step(cur, indent) {
        let lead = lead_comments(prev, cur.span.start, indent);
    ...
    let lead = lead_comments(prev, cur.span.start, indent);
    let tail = format!("{}{}", INDENT.repeat(indent), fmt_stmt(cur, indent));
    lines.push(if lead.is_empty() {
        tail
    } else {
        format!("{lead}{tail}")
...
fn block_step<'a>(cur: &'a S<Expr>, indent: usize) -> Option<BlockStep<'a>> {
    let ind = INDENT.repeat(indent);
    ...
    let (rendered, prev, next) = match &cur.node {
        Expr::Let(x, v, b) => {
            let s = if x == "_" {
                format!("{ind}{}", fmt_stmt(v, indent))
            } else {
                fmt_let_line(x, v, indent, cur.span.start)
            };
            (s, v.span.end, b.as_ref())
        }
        Expr::Sugar(Sugar::VarDecl(x, v, b)) => (
            format!("{ind}var {x} := {}", fmt_expr(v, indent, Mode::Flat)),
            v.span.end,
            b.as_ref(),
        ),
        Expr::Call(f, args) if is_with_call(args) => {
            ...
            let head = fmt_call_flat(f, init, indent);
            let bind = ps
                .first()
        ...
        Expr::Handle(body, arms) if cur.synth => (
            format!(
                "{ind}with handler\n{}",
                fmt_handler_arms(arms, indent + 1, cur.span.start)
            ),
        ...
        Expr::Match(s, arms) if cur.synth && arms.len() == 1 => (
            fmt_let_line(&fmt_pat_inline(&arms[0].pat), s, indent, cur.span.start),
            arms[0].body.span.start,
            &arms[0].body,
        ),
        Expr::Match(s, arms) if cur.synth && arms.len() == 2 => {
            let v = fmt_expr(s, indent, Mode::Flat);
            ...
            let s = binder.map_or_else(|| format!("{ind}{v}?"), |x| format!("{ind}let {x} = {v}?"));
...
fn fmt_trailing(e: &S<Expr>, indent: usize) -> Option<String> {
    ...
    let head = match dot_parts(f, init) {
        Some((name, recv, [])) => format!("{}.{name}", fmt_dot_recv(recv, indent)),
        _ => fmt_call_flat(f, init, indent),
    };
    let params = if ps.is_empty() {
        String::new()
    } else {
        let ps: Vec<String> = ps.iter().map(fmt_param).collect();
        format!("({})", ps.join(", "))
    };
    Some(format!(
        "{head} fn{params}\n{}",
        fmt_block(body, indent + 1, last.span.start)
    ))
}

// An `if`/`elif` chain whose final branch is `()` prints in statement
// position without the `else`, the open form it parsed from.
fn fmt_open_if(e: &S<Expr>, indent: usize) -> Option<String> {
    ...
    let parts: Vec<String> = arms
        ...
        .map(|(i, (c, t))| {
            ...
            format!(
                ...
                kw(i),
                fmt_head(c, indent),
                fmt_block(t, indent + 1, c.span.end)
            )
        })
        .collect();
    Some(parts.join("\n"))
...
fn fmt_stmt(e: &S<Expr>, indent: usize) -> String {
    ...
    fmt_expr_break(e, indent, Mode::Layout)
}

fn fmt_let_line(x: &str, v: &S<Expr>, indent: usize, from: usize) -> String {
    let ind = INDENT.repeat(indent);
    if !has_comments(from, v.span.end) && !forces_break(v) {
        if let Some(s) = fmt_expr_inline(v, Mode::Layout) {
    ...
    format!("{ind}let {x} =\n{}", fmt_block(v, indent + 1, from))
}

fn fmt_expr(e: &S<Expr>, indent: usize, mode: Mode) -> String {
    if let Some(s) = fmt_expr_inline(e, mode) {
        if indent * INDENT.len() + s.len() <= LINE_WIDTH {
            return s;
        }
    }
    fmt_expr_break(e, indent, mode)
}
...
fn fmt_interp(f: &S<Expr>, args: &[S<Expr>]) -> Option<String> {
    ...
    for (i, a) in args.iter().enumerate() {
        if i % 2 == 0 {
            ...
            let h = fmt_expr_inline(a, Mode::Flat).unwrap_or_else(|| {
                fmt_expr_break(a, 0, Mode::Flat)
                    .split_whitespace()
                    .collect::<Vec<_>>()
                    .join(" ")
            });
...
fn fmt_expr_inline(e: &S<Expr>, mode: Mode) -> Option<String> {
    match &e.node {
        ...
        Expr::Tuple(elems) => {
            let parts: Option<Vec<_>> = elems
                .iter()
                .map(|x| fmt_expr_inline(x, Mode::Flat))
                .collect();
        ...
        Expr::Bin(op, a, b) => {
            ...
            let a_s = fmt_expr_inline(a, if a_paren { Mode::Flat } else { mode })?;
            let b_s = fmt_expr_inline(b, if b_paren { Mode::Flat } else { mode })?;
        ...
        Expr::If(..) => {
            ...
            while let Expr::If(c, t, el) = &cur.node {
                ...
                let c = fmt_expr_inline(c, mode)?;
                let t = fmt_expr_inline(t, mode)?;
                parts.push(format!("{kw} {c} then {t}"));
                cur = el;
            }
            parts.push(format!("else {}", fmt_expr_inline(cur, mode)?));
            Some(parts.join(" "))
        }
        Expr::Call(f, args) => {
            if let Some(s) = fmt_interp(f, args) {
            ...
            if let Some(recv) = try_recv(f, args) {
                let recv_s = fmt_expr_inline(recv, Mode::Flat)?;
            ...
            if let Some((name, recv, rest)) = dot_parts(f, args) {
                ...
                let rest_s: Option<Vec<_>> = rest
                    .iter()
                    .map(|a| fmt_expr_inline(a, Mode::Flat))
                    .collect();
        ...
        Expr::Pipe(x, f) => {
            let x_s = fmt_expr_inline(x, mode)?;
            let f_s = fmt_expr_inline(f, mode)?;
            Some(format!("{x_s} |> {f_s}"))
        }
        Expr::Let(x, v, b) => {
            ...
            let v = fmt_expr_inline(v, mode)?;
            let b = fmt_expr_inline(b, mode)?;
        ...
        Expr::Lam(ps, body) => {
            ...
            let ps: Vec<_> = ps.iter().map(fmt_param).collect();
            let body = fmt_expr_inline(body, Mode::Flat)?;
        ...
        Expr::Match(s, arms) => {
            ...
            let s = fmt_expr_inline(s, mode)?;
            let arm_strs: Option<Vec<_>> = arms
                .iter()
                .map(|a| {
                    let p = fmt_pat_inline(&a.pat);
                    let g = match &a.guard {
                        Some(g) => format!(" if {}", fmt_expr_inline(g, mode)?),
                        None => String::new(),
                    };
                    let b = fmt_expr_inline(&a.body, mode)?;
                    Some(format!("{p}{g} => {b}"))
                })
                .collect();
        ...
        Expr::RecordCreate(name, fields) => {
            let fs: Option<Vec<_>> = fields
                .iter()
                .map(|(f, e)| fmt_expr_inline(e, Mode::Flat).map(|e_s| format!("{f} = {e_s}")))
                .collect();
            fs.map(|fs| format!("{name} {{ {} }}", fs.join(", ")))
        }
        Expr::RecordUpdate(base, name, fields) => {
            let base_s = fmt_expr_inline(base, Mode::Flat)?;
            let fs: Option<Vec<_>> = fields
                .iter()
                .map(|(f, e)| fmt_expr_inline(e, Mode::Flat).map(|e_s| format!("{f} = {e_s}")))
                .collect();
        ...
        Expr::RecordUpdatePath(base, ups) => {
            ...
            let us: Option<Vec<_>> = ups
                .iter()
                .map(|(p, e)| {
                    fmt_expr_inline(e, Mode::Flat).map(|e_s| format!("{} = {e_s}", p.join(".")))
                })
                .collect();
        ...
        Expr::Ann(inner, ty) => {
            let s = fmt_expr_inline(inner, Mode::Flat)?;
            Some(format!("({s} : {})", fmt_ty(ty)))
        }
        Expr::Mask(eff, body) => {
            let b = fmt_expr_inline(body, Mode::Flat)?;
            Some(format!("mask<{eff}>({b})"))
        }
        Expr::Sugar(s) => fmt_sugar_inline(s, mode),
...
fn fmt_sugar_inline(s: &Sugar<Surface>, mode: Mode) -> Option<String> {
    match s {
        Sugar::Default(a, b) => {
            let a_s = fmt_expr_inline(a, mode)?;
        ...
        Sugar::Assign(x, v) => {
            let v_s = fmt_expr_inline(v, mode)?;
            Some(format!("{x} := {v_s}"))
        }
        ...
        Sugar::TryCatch(body, arms) => {
            ...
            let parts: Option<Vec<_>> = arms
                .iter()
                .map(|a| {
                    fmt_expr_inline(&a.body, Mode::Flat).map(|s| {
        ...
        Sugar::For(x, s, quals, body) => {
            let s_s = fmt_expr_inline(s, Mode::Flat)?;
            let b = fmt_expr_inline(body, Mode::Flat)?;
            Some(format!("for {x} in {s_s}{} do {b}", fmt_quals(quals)?))
        }
        Sugar::Comp(head, x, s, quals) => {
            let h = fmt_expr_inline(head, Mode::Flat)?;
            let s_s = fmt_expr_inline(s, Mode::Flat)?;
            Some(format!("[{h} for {x} in {s_s}{}]", fmt_quals(quals)?))
        }
        Sugar::Transact(body, fallback) => {
            let b = fmt_expr_inline(body, Mode::Flat)?;
            let f = fmt_expr_inline(fallback, Mode::Flat)?;
            Some(format!("transact {b} else {f}"))
        }
        Sugar::Range(pre, hi) => {
            let parts: Option<Vec<_>> = pre.iter().map(|e| fmt_expr_inline(e, mode)).collect();
            let hi_s = fmt_expr_inline(hi, mode)?;
...
fn fmt_qual(q: &Qualifier, indent: usize) -> String {
    match q {
        Qualifier::Guard(g) => {
            let g =
                fmt_expr_inline(g, Mode::Flat).unwrap_or_else(|| fmt_expr(g, indent, Mode::Flat));
            format!("if {g}")
        }
        Qualifier::Bind(y, e) => {
            let e =
                fmt_expr_inline(e, Mode::Flat).unwrap_or_else(|| fmt_expr(e, indent, Mode::Flat));
            format!("let {y} = {e}")
        }
    }
}

fn fmt_quals(quals: &[Qualifier]) -> Option<String> {
    use std::fmt::Write;
    let mut out = String::new();
    for q in quals {
        match q {
            Qualifier::Guard(g) => write!(out, ", if {}", fmt_expr_inline(g, Mode::Flat)?).ok()?,
            Qualifier::Bind(y, e) => {
                write!(out, ", let {y} = {}", fmt_expr_inline(e, Mode::Flat)?).ok()?;
            }
        }
    }
    Some(out)
...
fn fmt_head(e: &S<Expr>, indent: usize) -> String {
    if let Some(s) = fmt_expr_inline(e, Mode::Flat) {
        if indent * INDENT.len() + s.len() + 16 <= LINE_WIDTH {
            return s;
        }
    }
    format!("({})", fmt_expr_break(e, indent + 1, Mode::Flat))
}

// `from` is the offset just past the arm's `=>`, so a comment on the line
// below the arrow re-emits as the body's leading comment.
fn fmt_arm_body(b: &S<Expr>, indent: usize, used: usize, from: usize) -> String {
    if !forces_break(b) && !has_comments(from, b.span.end) {
        if let Some(s) = fmt_expr_inline(b, Mode::Layout) {
            if used + 1 + s.len() <= LINE_WIDTH {
                return format!(" {s}");
            }
        }
    }
    format!("\n{}", fmt_block(b, indent + 1, from))
}

fn fmt_handler_arms(arms: &[HandlerArm], indent: usize, from: usize) -> String {
    let ind = INDENT.repeat(indent);
    ...
    for arm in arms {
        ...
        let lead = lead_comments(prev, body.span.start, indent);
        let rendered = format!(
            "{head}{}",
            fmt_arm_body(body, indent, head.len(), body.span.start)
        );
...
fn fmt_guard(a: &Arm, indent: usize) -> String {
...
fn fmt_expr_break(e: &S<Expr>, indent: usize, mode: Mode) -> String {
    match (&e.node, mode) {
        (Expr::Match(scrut, arms), Mode::Layout) => fmt_match_layout(scrut, arms, indent),
        (Expr::Match(s, arms), _) => fmt_match_flat(s, arms, indent, mode),
        (Expr::If(c, t, el), Mode::Layout) => fmt_if_layout(c, t, el, indent),
        (Expr::If(c, t, el), _) => fmt_if_flat(c, t, el, indent, mode),
        (Expr::Let(..), Mode::Layout) => format!("({})", fmt_expr(e, indent + 1, Mode::Flat)),
        (Expr::Let(x, v, b), _) => fmt_let_break(x, v, b, indent, mode),
        (Expr::Pipe(x, f), _) => fmt_pipe_break(x, f, indent, mode),
        (Expr::Call(f, args), _) => fmt_call_flat(f, args, indent),
        (Expr::Handle(body, arms), Mode::Layout) => fmt_handle_layout(body, arms, indent),
        (Expr::Sugar(Sugar::TryCatch(body, arms)), Mode::Layout) => {
            fmt_trycatch_layout(e, body, arms, indent)
        }
        (Expr::Sugar(Sugar::For(x, s, quals, body)), Mode::Layout) => {
            fmt_for_layout(x, s, quals, body, indent)
        }
        (Expr::Sugar(Sugar::Transact(body, fallback)), Mode::Layout) => {
            fmt_transact_layout(e, body, fallback, indent)
        }
        (Expr::Sugar(Sugar::VarDecl(..) | Sugar::NamedHandle(..)), _) => {
            fmt_block(e, indent, e.span.start).trim_start().to_string()
        }
        (Expr::Sugar(Sugar::Assign(x, v)), _) => {
            format!("{x} := {}", fmt_expr(v, indent, Mode::Flat))
        }
        (Expr::Handle(body, arms), _) => fmt_handle_flat(body, arms, indent, mode),
        _ => fmt_expr_inline(e, Mode::Flat).unwrap_or_else(|| verbatim(e.span.start, e.span.end)),
    }
}

fn fmt_match_layout(scrut: &S<Expr>, arms: &[Arm], indent: usize) -> String {
    let s = fmt_head(scrut, indent);
    ...
    let mut arm_strs: Vec<String> = Vec::with_capacity(arms.len());
    for a in arms {
        let lead = lead_comments(prev, a.pat.span.start, indent + 1);
        let head = format!(
            "{}{}{} =>",
            INDENT.repeat(indent + 1),
            fmt_pat(&a.pat, indent + 1),
            fmt_guard(a, indent + 1)
        );
        let from = a.guard.as_ref().map_or(a.pat.span.end, |g| g.span.end);
        let body = fmt_arm_body(&a.body, indent + 1, head.len(), from);
...
fn fmt_match_flat(s: &S<Expr>, arms: &[Arm], indent: usize, mode: Mode) -> String {
    let ind = INDENT.repeat(indent);
    let ind1 = INDENT.repeat(indent + 1);
    let s = fmt_expr_inline(s, mode).unwrap_or_else(|| fmt_expr(s, indent, mode));
    let n = arms.len();
    let arm_strs: Vec<String> = arms
        ...
        .map(|(i, a)| {
            ...
            let b_inline = fmt_expr_inline(&a.body, mode);
            ...
            let b_break = fmt_expr_break(&a.body, indent + 2, mode);
            format!("{ind1}{p} =>\n{ind2}{b_break}{trail}")
        })
        .collect();
    format!("match {s} of {{\n{}\n{ind}}}", arm_strs.join("\n"))
...
fn fmt_if_layout(c: &S<Expr>, t: &S<Expr>, el: &S<Expr>, indent: usize) -> String {
    let ind = INDENT.repeat(indent);
    ...
    while let Expr::If(c2, t2, e2) = &cur.node {
        parts.push(format!(
            "{ind}elif {} then\n{}",
            fmt_head(c2, indent),
            fmt_block(t2, indent + 1, c2.span.end)
        ));
    ...
    parts.push(format!(
        "{ind}else\n{}",
        fmt_block(cur, indent + 1, last_then)
    ));
    parts.join("\n")
}

fn fmt_if_flat(c: &S<Expr>, t: &S<Expr>, el: &S<Expr>, indent: usize, mode: Mode) -> String {
    let ind1 = INDENT.repeat(indent + 1);
    let c = fmt_expr_inline(c, mode).unwrap_or_else(|| fmt_expr(c, indent, mode));
    let t = fmt_expr(t, indent + 1, mode);
    let el = fmt_expr(el, indent + 1, mode);
    format!("if {c}\n{ind1}then {t}\n{ind1}else {el}")
}

fn fmt_let_break(x: &str, v: &S<Expr>, b: &S<Expr>, indent: usize, mode: Mode) -> String {
    let ind = INDENT.repeat(indent);
    let v = fmt_expr(v, indent, mode);
    let b = fmt_expr(b, indent, mode);
    format!("let {x} = {v} in\n{ind}{b}")
}

fn fmt_pipe_break(x: &S<Expr>, f: &S<Expr>, indent: usize, mode: Mode) -> String {
    let ind = INDENT.repeat(indent);
    let x_s = if mode == Mode::Layout && matches!(x.node, Expr::Let(..)) {
        format!("({})", fmt_expr(x, indent + 1, Mode::Flat))
    } else {
        fmt_expr(x, indent, mode)
    };
    let f_s = fmt_expr_inline(f, Mode::Flat).unwrap_or_else(|| fmt_expr(f, indent + 1, Mode::Flat));
    format!("{x_s}\n{ind}  |> {f_s}")
}

fn fmt_handle_layout(body: &S<Expr>, arms: &[HandlerArm], indent: usize) -> String {
    let body_s = fmt_head(body, indent);
    format!(
        "handle {body_s} with\n{}",
        fmt_handler_arms(arms, indent + 1, body.span.end)
    )
}

fn fmt_trycatch_layout(e: &S<Expr>, body: &S<Expr>, arms: &[CatchArm], indent: usize) -> String {
    let ind = INDENT.repeat(indent);
    let ind1 = INDENT.repeat(indent + 1);
    ...
    let mut arm_strs: Vec<String> = Vec::with_capacity(arms.len());
    for a in arms {
        let lead = lead_comments(prev, a.span.start, indent + 1);
        let head = if a.binders.is_empty() {
            format!("{ind1}{} =>", a.name)
        } else {
            format!("{ind1}{}({}) =>", a.name, a.binders.join(", "))
        };
        let arm_body = fmt_arm_body(&a.body, indent + 1, head.len(), a.body.span.start);
        arm_strs.push(format!("{lead}{head}{arm_body}"));
        prev = a.body.span.end;
    }
    format!(
        "try\n{}\n{ind}catch\n{}",
        fmt_block(body, indent + 1, e.span.start),
        arm_strs.join("\n")
    )
}
...
fn fmt_for_layout(
    ...
    format!(
        "{} do\n{}",
        parts.join(",\n"),
        fmt_block(body, indent + 1, from)
    )
}

fn fmt_transact_layout(e: &S<Expr>, body: &S<Expr>, fallback: &S<Expr>, indent: usize) -> String {
    let ind = INDENT.repeat(indent);
    format!(
        "transact\n{}\n{ind}else\n{}",
        fmt_block(body, indent + 1, e.span.start),
        fmt_block(fallback, indent + 1, body.span.end)
    )
}

fn fmt_handle_flat(body: &S<Expr>, arms: &[HandlerArm], indent: usize, mode: Mode) -> String {
    let ind = INDENT.repeat(indent);
    let ind1 = INDENT.repeat(indent + 1);
    let body_s = fmt_expr(body, indent, mode);
    let arm_strs: Vec<String> = arms
        .iter()
        .map(|arm| match arm {
            HandlerArm::Return(x, arm_body) => {
                format!(
                    "{ind1}return {x} => {}",
                    fmt_expr(arm_body, indent + 1, mode)
                )
            }
            ...
            HandlerArm::Sugar(SugarArm::Val(x, arm_body)) => {
                format!("{ind1}val {x} = {}", fmt_expr(arm_body, indent + 1, mode))
            }
        })
        .collect();
