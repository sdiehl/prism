...
use super::{Lowerer, ResumeCtorState, ResumeMode, EOP, EPURE, ERESUME, OP_TAG, QAPPLY};
...
impl Lowerer {
    ...
    // An open (whole-program) handler falls to the re-enterable thunk driver: the
    // free-monad lowering whose cost is constant per live continuation.
    ...
    pub(super) fn lower_handle_native(&mut self, c: &Comp) -> Result<Comp, TypeError> {
        ...
        self.resume_ctor = ResumeCtorState::Used;
