...
// misorder one element type. This gate checks the correspondence end to end: for
...
fn check(label: &str, full: &str) -> Result<(), String> {
    ...
    if got != want {
        return Err(format!(
            r"{label}: native disagrees with interpreter (a drifted kind tag):
  native: {got:?}
  interp: {want:?}"
        ));
    }
