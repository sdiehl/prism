...
const P_EDIT_LEAF: &str = "\
fn oc_leaf(n : Int) : Int = n + 2
fn oc_mid(n : Int) : Int = oc_leaf(n) * 2
fn oc_top(n : Int) : Int =
  let m = oc_mid(n)
  m + oc_leaf(n)
...
";
const P_SHOW: &str = "\
fn describe(n : Int) : String = show(n)
...
// and this fails. It pins the two sites to one regime so they can never silently
// fork again.
#[test]
    ...
    for prog in [P_NEWTYPE, P_SHOW] {
        ...
        for (sym, hash) in &recomputed {
            assert!(
                ...
                "definition `{}` recomputes to pre-optimizer hash {hash}, which the \
                 store never committed: the commit regime has forked from \
                 store_def_inputs (both must hash pre-optimizer elaborated Core)",
                sym.as_str()
            );
        }
    }
