...
fn every_lexer_keyword_is_documented() {
    ...
    assert!(
        missing.is_empty(),
        "grammar.ebnf drift: the lexer reserves these words but the ebnf never \
         mentions them as terminals: {missing:?}. Document each in \
         models/grammar.ebnf (or, if intentionally undocumented, note why)."
    );
}
