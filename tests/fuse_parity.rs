...
// against the interpreter, the same anchor `tests/parity.rs` pins the fuse-OFF
...
// determinism check pins the byte-stable join naming the anti-unifier promises.
...
fn run_touched(tag: &str, build: impl Fn(&str, &Path) -> Result<(), prism::Error> + Sync) {
    ...
    assert!(
        cases.len() >= 6,
        "fusion oracle exercises only {} programs (floor 6); the fusion corpus or \
         recognition likely broke",
        cases.len()
    );
