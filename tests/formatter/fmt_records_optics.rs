// Snapshot tests for the multi-line layout of record constructors and nested
        ...
        fn $name() {
            insta::with_settings!({
                ...
                insta::assert_snapshot!(
                    concat!("fmt_records_optics__", stringify!($name)),
                    fmt($src)
                );
            });
        }
...
case!(
    ...
    r"fn mk() : A =
  A { inner = B { inner = C { p = 1, q = 2 }, y = 2 }, z = 3 }
"
);
...
case!(
    ...
    r"fn bump(p : Player) : Player =
  Player { ..p, name = q, pos = Vec2 { x = 0, y = 0 }, hp = 100, mp = 50 }
"
);
...
case!(
    ...
    r"fn tick(world : World) : World =
  { world | players.each.hp ~ heal, players.each.mana = full, log = [] }
"
);
...
case!(
    optic_where_filter,
    r"fn hurt(army : Army) : Army =
  { army | units.each.(hp where alive) ~ dec, turn = next }
"
);
...
case!(
    ...
    r"fn f(g : Game) : Game =
  { g | items[0].?Weapon.dmg ~ boost, players.each.slot[1] = empty }
"
);
...
case!(
    ...
    r"fn f(g : Game) : Game =
  { g | board.each.cell = Cell { a = 1, b = 2, c = 3, d = 4, e = 5 }, v = 0 }
"
);
