...
//! the emitted graph. The library-level case checks the replay contract directly: a
...
const WRITE_PROGRAM: &str = "fn main() : Unit ! {IO} =\n  \
    let cfg = read_file(\"input.json\")\n  \
...
// Recording a run and replaying its trace produce the identical provenance
...
const PURE_PROGRAM: &str =
    "fn main() : Unit ! {IO} = println(\"{args_count()} {rand_below(100)}\")\n";
...
fn mismatched_replay_names_the_event_index_and_expected_operation() {
    const STRING_FIRST: &str = "fn main() : Unit ! {IO} = println(getenv(\"X\"))\n";
...
// The replay contract extends to writes: a program that writes a file re-executes the write on
