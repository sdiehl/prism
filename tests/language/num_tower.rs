...
// lane chosen by the type and resolved entirely at compile time. These check the
...
// guarded by the parity oracle; these are the typing checks.
    ...
    const fn src(self) -> &'static str {
        match self {
            ...
            Self::I64ListLiteralContext => {
                r"fn sums(xs : List(I64)) : I64 = foldl(\(a, x) -> a + x, 0i64, xs)
...
fn generic_ord_comparisons_check_at_every_lane() {
    let prog = r#"fn lt(x : a, y : a) : Bool given Ord(a) = x < y
fn main() =
  println(show(lt(1, 2)))
...
fn one_source_runs_at_two_lanes() {
    let prog = r"fn dbl(x : a) : a given Num(a) = x + x
fn main() =
  println(show(dbl(21)))
...
fn literal_adopts_a_num_polymorphic_variable() {
    let prog = r"fn scale2(x : a) : a given Num(a) = x * 2 + 1
fn main() =
...
fn negated_literal_adopts_a_num_polymorphic_variable() {
    let prog = r"fn dec(x : a) : a given Num(a) = x + -1
fn main() =
  println(show(dec(10)))
