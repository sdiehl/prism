...
mutual
  ...
  inductive Frame where
    ...
    | handle (ops : List HandleOp) (retVar : Option String)
             (retBody : Option Comp) (env : List (String × Rv))
...
def matchPatR : Pat → Rv → Option (List (String × Rv))
  ...
  | .ctor name args, .data name' _ vs =>
      if name = name' then matchPatRL args vs else none
...
def findHandler (op : String) (argvs : List Rv) :
    Nat → List Frame → Stack → Option Conf
  | _, _, [] => none
...
def step (Γ : Core) : Conf → Option Conf
  | (.eval c env, stk) =>
    ...
    | .force v => match atomEval env v with
    ...
    | .ite cnd t e => match atomEval env cnd with
      | some (.bool true) => some (.eval t env, stk)
      | some (.bool false) => some (.eval e env, stk)
      | _ => none
    | .prim op a b => match atomEval env a, atomEval env b with
    ...
    | .call name args =>
      ...
      | some f, some avs =>
          if avs.length < f.params.length then
            some (.ret (.closure (f.params.drop avs.length) f.body (extendEnv f.params avs [])), stk)
          else
            some (.eval f.body (extendEnv f.params avs []), stk)
      | _, _ => none
    | .case scrut arms => match atomEval env scrut with
      | some sv => match matchArmsR sv arms with
        | some (body, binds) => some (.eval body (binds ++ env), stk)
        | none => none
      | none => none
    | .doOp op args => match atomEval.atomEvalL env args with
  ...
  | (.ret w, stk) =>
    ...
    | fr :: rest =>
      ...
      | .handle _ rv rb env => match rv, rb with
        | some r, some body => some (.eval body ((r, w) :: env), rest)
        | _, _ => some (.ret w, rest)
      | .args args env => match atomEval.atomEvalL env args with
        | some avs => match w with
          | .closure ps body cenv =>
              if avs.length < ps.length then
                some (.ret (.closure (ps.drop avs.length) body (extendEnv ps avs cenv)), rest)
              else
                some (.eval body (extendEnv ps avs cenv), rest)
          | .resume frames => match avs with
...
theorem MStep.deterministic {Γ : Core} {a b c : Conf}
    (h1 : MStep Γ a b) (h2 : MStep Γ a c) : b = c := by
  unfold MStep at h1 h2; rw [h1] at h2; exact Option.some.inj h2
...
theorem Runs.trans {Γ : Core} {a b c : Conf}
    (hab : Runs Γ a b) (hbc : Runs Γ b c) : Runs Γ a c := by
  induction hab with
  | refl => exact hbc
...
inductive BEval (Γ : Core) : Env → Comp → Rv → Prop where
  ...
  | appFull : BEval Γ env f (.closure ps body cenv) →
      atomEval.atomEvalL env args = some avs → ¬ avs.length < ps.length →
      BEval Γ (extendEnv ps avs cenv) body w → BEval Γ env (.app f args) w
  | appPart : BEval Γ env f (.closure ps body cenv) →
      atomEval.atomEvalL env args = some avs → avs.length < ps.length →
      BEval Γ env (.app f args) (.closure (ps.drop avs.length) body (extendEnv ps avs cenv))
  ...
  | prim : atomEval env a = some av → atomEval env b = some bv → deltaR op av bv = some w →
      BEval Γ env (.prim op a b) w
  | callFull : lookupFn Γ name = some f → atomEval.atomEvalL env args = some avs →
      ¬ avs.length < f.params.length →
      BEval Γ (extendEnv f.params avs []) f.body w → BEval Γ env (.call name args) w
  | callPart : lookupFn Γ name = some f → atomEval.atomEvalL env args = some avs →
      avs.length < f.params.length →
      BEval Γ env (.call name args) (.closure (f.params.drop avs.length) f.body (extendEnv f.params avs []))
  | case : atomEval env scrut = some sv → matchArmsR sv arms = some (body, binds) →
      BEval Γ (binds ++ env) body w → BEval Γ env (.case scrut arms) w
...
theorem bigstep_runs {Γ : Core} {env : Env} {c : Comp} {w : Rv}
    (h : BEval Γ env c w) : ∀ S, Runs Γ (.eval c env, S) (.ret w, S) := by
  induction h with
  | ret hv => intro S; exact Runs.step (by simp [step, hv]) .refl
  | bind _ _ ihm ihn =>
      intro S
  ...
  | forceThunk hv _ ih =>
      intro S; exact Runs.step (by simp [step, hv]) (ih S)
  | forceVal hv hne =>
      intro S
      refine Runs.step ?_ .refl
      revert hv hne
      cases w <;> intro hv hne <;>
        first
          | exact absurd rfl (hne _ _)
          | simp [step, hv]
  | lam => intro S; exact Runs.step (by simp [step]) .refl
  ...
  | appPart _ hargs hlen ihf =>
      ...
      refine Runs.step ?_ .refl
      simp [step, hargs, hlen]
  | iteT hc _ ih => intro S; exact Runs.step (by simp [step, hc]) (ih S)
  | iteF hc _ ih => intro S; exact Runs.step (by simp [step, hc]) (ih S)
  | prim ha hb hd => intro S; exact Runs.step (by simp [step, ha, hb, hd]) .refl
  | callFull hf hargs hlen _ ihb =>
      intro S; exact Runs.step (by simp [step, hf, hargs, hlen]) (ihb S)
  | callPart hf hargs hlen =>
      intro S; exact Runs.step (by simp [step, hf, hargs, hlen]) .refl
  | case hs hm _ ih => intro S; exact Runs.step (by simp [step, hs, hm]) (ih S)
  | withReuse _ ih => intro S; exact Runs.step (by simp [step]) (ih S)
  | reuse hv => intro S; exact Runs.step (by simp [step, hv]) .refl
  | dup => intro S; exact Runs.step (by simp [step]) .refl
  | drop => intro S; exact Runs.step (by simp [step]) .refl
  | print => intro S; exact Runs.step (by simp [step]) .refl
  | printf => intro S; exact Runs.step (by simp [step]) .refl
  | prints => intro S; exact Runs.step (by simp [step]) .refl
  | readInt => intro S; exact Runs.step (by simp [step]) .refl
  | floatBuiltin => intro S; exact Runs.step (by simp [step]) .refl
  | strBuiltin => intro S; exact Runs.step (by simp [step]) .refl

/-- Oracle adequacy on closed programs: the natural semantics and the machine
    agree on the final value. -/
theorem load_runs {Γ : Core} {c : Comp} {w : Rv}
