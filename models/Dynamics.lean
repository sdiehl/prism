...
inductive Handles (op : String) : Nat → Stack → Prop where
  | here (ops rv rb env rest) :
      handlerFor op ops ≠ none → Handles op 0 (.handle ops rv rb env :: rest)
  | skipH (ops rv rb env rest skip) :
      handlerFor op ops ≠ none → Handles op skip rest →
  ...
  | maskYes (ops rest skip) :
      ops.contains op → Handles op (skip + 1) rest → Handles op skip (.mask ops :: rest)
  | maskNo (ops rest skip) :
      ¬ ops.contains op → Handles op skip rest → Handles op skip (.mask ops :: rest)
...
theorem effect_progress {op : String} {argvs : List Rv} {skip : Nat} {stk : Stack}
    (h : Handles op skip stk) :
    ∀ captured, (findHandler op argvs skip captured stk).isSome := by
  induction h with
  ...
  | skipH ops rv rb env rest skip hf _ ih =>
      ...
      cases hfm : handlerFor op ops with
      | none => exact absurd hfm hf
      | some t => simp [findHandler, hfm]; exact ih _
  | passH ops rv rb env rest skip hf _ ih =>
      intro captured; simp [findHandler, hf]; exact ih _
  | maskYes ops rest skip hc _ ih =>
      intro captured; simp only [findHandler]; rw [if_pos hc]; exact ih _
  | maskNo ops rest skip hc _ ih =>
      intro captured; simp only [findHandler]; rw [if_neg hc]; exact ih _
  | bind x n env rest skip _ ih => intro captured; simp [findHandler]; exact ih _
  | args a env rest skip _ ih => intro captured; simp [findHandler]; exact ih _
...
theorem effect_unhandled {op : String} {argvs : List Rv} :
    ∀ {skip : Nat} {stk : Stack}, ¬ Handles op skip stk →
    ∀ captured, findHandler op argvs skip captured stk = none := by
  intro skip stk
  induction stk generalizing skip with
  | nil => intro _ captured; rfl
  | cons fr rest ih =>
    intro hns captured
    cases fr with
    | bind x n env =>
    ...
    | handle ops rv rb env =>
        ...
        cases hfm : handlerFor op ops with
        | none =>
            exact ih (fun hr => hns (.passH ops rv rb env rest skip hfm hr)) _
        | some t =>
            cases skip with
            | zero => exact absurd (Handles.here ops rv rb env rest (by rw [hfm]; exact nofun)) hns
            | succ k =>
                simp only [Nat.succ_sub_one]
                exact ih (fun hr => hns (.skipH ops rv rb env rest k (by rw [hfm]; exact nofun) hr)) _
