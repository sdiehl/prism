...
/-
This is the small bag of proof facts around `Prism.lean`. The core step relation
is deterministic, and this file lifts that into the usual things you want when
debugging a semantics.

Runs have unique terminal answers. Batch substitution unfolds the way it should.
Every term is either already terminal, able to step, or explicitly stuck. That
last case matters because this model is untyped Core; bad programs are allowed
to exist, but they should get stuck for a reason we can name.
-/
namespace Prism
...
/-- Adding one substitution to the front means performing it first, then
    continuing with the rest. -/
@[simp]
theorem substMany_cons
    (x : String) (w : Value) (rest : List (String × Value)) (c : Comp) :
    substMany ((x, w) :: rest) c = substMany rest (substC x w c) :=
  rfl

/-- A lambda absorbs substitution of one of its own parameters. -/
theorem substC_lam_shadow
    {x : String} {w : Value} {xs : List String} {b : Comp}
    (h : xs.contains x) : substC x w (.lam xs b) = .lam xs b :=
  by
...
inductive Stuck (Γ : Core) : Comp → Prop where
  ...
  | callMissing (name : String) (args : List Value) :
      lookupFn Γ name = none →
      Stuck Γ (.call name args)
  | caseNoMatch (s : Value) (arms : List (Pat × Comp)) :
      matchArms s arms = none →
      Stuck Γ (.case s arms)
...
/-- A force either unwraps a thunk or is reported stuck. -/
...
/-- A conditional either selects the matching branch or is stuck on a non-bool
    condition. -/
theorem iteP (Γ : Core) (c : Value) (t e : Comp) :
    (∃ c', Step Γ (.ite c t e) c') ∨ Stuck Γ (.ite c t e) :=
  by cases c with
...
/-- A binary primitive either computes a result or is stuck on unsupported
    operands. -/
theorem primP (Γ : Core) (op : BinOp) (a b : Value) :
    (∃ c', Step Γ (.prim op a b) c') ∨ Stuck Γ (.prim op a b) :=
  by cases h : delta op a b with
      | some v => exact Or.inl ⟨_, .prim h⟩
      | none => exact Or.inr (.primStuck _ _ _ h)

/-- A numeric negation either computes a result or is stuck on the wrong lane or
    operand. -/
theorem negP (Γ : Core) (lane : NegLane) (v : Value) :
    (∃ c', Step Γ (.neg lane v) c') ∨ Stuck Γ (.neg lane v) :=
  by cases h : negD lane v with
      | some w => exact Or.inl ⟨_, .neg h⟩
      | none => exact Or.inr (.negStuck _ _ h)

/-- A named call either expands through the core table or is stuck if the name is
    missing. -/
theorem callP (Γ : Core) (name : String) (args : List Value) :
    (∃ c', Step Γ (.call name args) c') ∨ Stuck Γ (.call name args) :=
  by cases h : lookupFn Γ name with
      | some f => exact Or.inl ⟨_, .call h⟩
      | none => exact Or.inr (.callMissing _ _ h)

/-- A case expression either finds a matching arm or is stuck with no match. -/
theorem caseP (Γ : Core) (s : Value) (arms : List (Pat × Comp)) :
    (∃ c', Step Γ (.case s arms) c') ∨ Stuck Γ (.case s arms) :=
...
assumed, so "stuck" genuinely occurs, e.g. `prim add true 1`; the theorem says
exactly where.)
-/
