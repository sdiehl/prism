...
theorem terminalNoStep {Γ : Core} {a b : Comp} (h : Terminal a) : ¬ Step Γ a b := by
  intro hs
  cases a <;> first
    | exact noStepRet hs
    | exact noStepLam hs
...
theorem Steps.unique_normal {Γ : Core} {a m n : Comp}
    (hm : Steps Γ a m) (hn : Steps Γ a n) (tm : Terminal m) (tn : Terminal n) : m = n := by
...
@[simp] theorem substMany_nil (c : Comp) : substMany [] c = c := rfl

@[simp] theorem substMany_cons (x : String) (w : Value) (rest : List (String × Value)) (c : Comp) :
    substMany ((x, w) :: rest) c = substMany rest (substC x w c) := rfl

/-- A lambda absorbs substitution of one of its own parameters. -/
theorem substC_lam_shadow {x : String} {w : Value} {xs : List String} {b : Comp}
    (h : xs.contains x) : substC x w (.lam xs b) = .lam xs b := by
...
theorem stuckNotTerminal {Γ : Core} {c : Comp} (h : Stuck Γ c) : ¬ Terminal c := by
  cases h <;> intro ht <;> exact ht.elim
...
theorem stuckNoStep {Γ : Core} {c c' : Comp} (h : Stuck Γ c) : ¬ Step Γ c c' := by
  induction h generalizing c' with
  | forceNonThunk v hne => intro hs; cases hs; exact hne _ rfl
  | iteNonBool c t e hne => intro hs; cases hs <;> exact hne _ rfl
  | primStuck op a b hd => intro hs; cases hs; rename_i hd'; simp [hd] at hd'
  | callMissing name args hl => intro hs; cases hs; rename_i hl'; simp [hl] at hl'
  | caseNoMatch s arms hm => intro hs; cases hs; rename_i hm'; simp [hm] at hm'
  | err v => intro hs; cases hs
  | doOp n args => intro hs; cases hs
  | handle b rv rb ops => intro hs; cases hs
  | mask ops b => intro hs; cases hs
  | print v => intro hs; cases hs
  | printf v => intro hs; cases hs
  | prints v => intro hs; cases hs
  | readInt => intro hs; cases hs
  | floatBuiltin n v => intro hs; cases hs
  | strBuiltin n args => intro hs; cases hs
  | bindLam xs b x n => intro hs; cases hs; rename_i hbad; exact noStepLam hbad
  | appRet v args => intro hs; cases hs; rename_i hbad; exact noStepRet hbad
...
theorem terminalCases {c : Comp} (h : Terminal c) :
    (∃ v, c = .ret v) ∨ (∃ xs b, c = .lam xs b) := by
  revert h; cases c <;> intro h <;> first
    | exact Or.inl ⟨_, rfl⟩
    | exact Or.inr ⟨_, _, rfl⟩
    | exact h.elim

theorem forceP (Γ : Core) (v : Value) :
    (∃ c', Step Γ (.force v) c') ∨ Stuck Γ (.force v) := by
  cases v <;> first
    | exact Or.inl ⟨_, .forceThunk⟩
    | exact Or.inr (.forceNonThunk _ (fun _ => nofun))

theorem iteP (Γ : Core) (c : Value) (t e : Comp) :
    (∃ c', Step Γ (.ite c t e) c') ∨ Stuck Γ (.ite c t e) := by
  cases c with
...
theorem primP (Γ : Core) (op : BinOp) (a b : Value) :
    (∃ c', Step Γ (.prim op a b) c') ∨ Stuck Γ (.prim op a b) := by
  cases h : delta op a b with
  | some v => exact Or.inl ⟨_, .prim h⟩
  | none => exact Or.inr (.primStuck _ _ _ h)

theorem callP (Γ : Core) (name : String) (args : List Value) :
    (∃ c', Step Γ (.call name args) c') ∨ Stuck Γ (.call name args) := by
  cases h : lookupFn Γ name with
  | some f => exact Or.inl ⟨_, .call h⟩
  | none => exact Or.inr (.callMissing _ _ h)

theorem caseP (Γ : Core) (s : Value) (arms : List (Pat × Comp)) :
    (∃ c', Step Γ (.case s arms) c') ∨ Stuck Γ (.case s arms) := by
  cases h : matchArms s arms with
...
-/
theorem progress (Γ : Core) :
    (c : Comp) → Terminal c ∨ (∃ c', Step Γ c c') ∨ Stuck Γ c
  ...
  | .bind m x n => match progress Γ m with
      | Or.inl tm => match terminalCases tm with
          | Or.inl ⟨_, hv⟩ => by subst hv; exact Or.inr (Or.inl ⟨_, .bindRet⟩)
          | Or.inr ⟨_, _, hl⟩ => by subst hl; exact Or.inr (Or.inr (.bindLam _ _ _ _))
      | Or.inr (Or.inl ⟨_, hs⟩) => Or.inr (Or.inl ⟨_, .bindCong hs⟩)
      | Or.inr (Or.inr hst) => Or.inr (Or.inr (.bindStuck _ _ _ hst))
  | .app f args => match progress Γ f with
      | Or.inl tf => match terminalCases tf with
          | Or.inl ⟨_, hv⟩ => by subst hv; exact Or.inr (Or.inr (.appRet _ _))
          | Or.inr ⟨_, _, hl⟩ => by subst hl; exact Or.inr (Or.inl ⟨_, .beta⟩)
