...
example : Step emptyΓ (.ite (.bool true) (.ret (.int 1)) (.ret (.int 2))) (.ret (.int 1)) :=
  .ifTrue
...
example : Steps viewΓ viewProg (.ret (.int 7)) :=
  .head (.bindCong (.call rfl)) (.head (.bindCong (.caseMatch rfl)) (.head .bindRet (.head (.caseMatch rfl) .refl)))
...
private def stripTrailing (s : String) : String :=
  if s.any (· == '.') then String.ofList (s.toList.reverse.dropWhile (· == '0') |>.dropWhile (· == '.')).reverse else s
...
def fmtG (d : Float) : String :=
  ...
  if expBits == 2047 then
    ...
    if M == 0 then
      ...
      let digitsAt : Nat → (Nat × Int) := fun P => let q : Int := (scale : Int) - ((P : Int) - 1 - e10₀)
      let R₀ : Nat := if q ≤ 0 then
        num * 10 ^ (-q).toNat
      else
        let p10 := 10 ^ q.toNat
        let quot := num / p10
        let rem := num % p10
        let half := p10 / 2
        if rem < half then quot else if rem > half then quot + 1 else (if quot % 2 == 0 then quot else quot + 1)
      if (toString R₀).length > P then (R₀ / 10, e10₀ + 1) else (R₀, e10₀)
      let dmag := Float.ofBits (bits &&& 9223372036854775807)
      let roundTrips : Nat → Bool := fun P => let (R, e10) := digitsAt P
      let exp : Int := e10 - ((P : Int) - 1)
      let cand := if exp ≥ 0 then Float.ofScientific R false exp.toNat else Float.ofScientific R true (-exp).toNat
...
def eval (Γ : Core) (fuel : Nat) (c : Comp) : String := showState (run fuel Γ (load c)).1
...
def evalCoreJson (src : String) : Except String String := Json.coreOfJson src |>.map evalMain
...
def main (argv : List String) : IO UInt32 := do
  match argv with
    ...
    | [] =>
      ...
      return 0
    | _ =>
      IO.eprintln "usage: oracle [eval <file.json|->]"
      return 1
