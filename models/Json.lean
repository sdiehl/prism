...
private def jStr (j : Json) (k : String) : Except String String := (j.getObjVal? k) >>= (·.getStr?)

private def jOpt (j : Json) (k : String) : Option Json := (j.getObjVal? k).toOption
...
partial def jComp (j : Json) : Except String Comp := do
  match (← jStr j "c") with
    ...
    | "bind" => return .bind (← jComp (← j.getObjVal? "m")) (← jStr j "x") (← jComp (← j.getObjVal? "n"))
    ...
    | "handle" =>
      ...
      let retBody ← match jOpt j "retBody" with
        | some b => some <$> jComp b
        | none => pure none
      return .handle (← jComp (← j.getObjVal? "body")) retVar retBody (← jList jHandleOp (← j.getObjVal? "ops"))
