...
ROWS = [
    ...
    Row(
        ...
        " coverage named above in between. Re-measured 2026-08-25 on the shipped"
        " tree with the fixture set named for once and a quiet box (stdev under"
        " 0.02 s over 20 runs): `tests/fixtures/bootstrap/t1.pr` alone is 2.11 s"
        " median, reproducing that single-fixture 2.29 s reading, and the full"
        " committed workbench `t1`+`t2`+`t3`, 556 surface nodes with every fixture"
        " at parity, is 4.68 s, the difference being the fixtures checked and not a"
        " slowdown. The pair that produced the 54x, 625 ms"
...
COST_PROVENANCE = (
    ...
    " nothing across rows. Re-run 2026-08-25 on the shipped v0.20.0 tree (the"
    " squash commit `92392b70`, the same tree these numbers were transcribed"
    " from) on the same machine class: every row reproduced within box noise, the"
    " structured-layer ratios and the peak resident set everywhere unchanged, so"
    " the table is confirmed as the shipped tree's reading rather than an artifact"
    " of a pre-cut checkout. The comment-dominated class's ratio wanders by a few"
    " x between runs by construction, its Rust side being microsecond-fast, and"
    " the rows are left at their measured 2026-08-23 values rather than overwritten"
    " with a noisier same-tree re-read."
)
