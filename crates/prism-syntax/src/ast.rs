...
pub struct Program<P: Phase = Surface> {
    ...
    // Name resolution reads this to give the two regions separate scopes, so a
    // user definition shadows a prelude one instead of replacing it. The parser
    // is the only writer, from the same `SourceMap` boundary the diagnostic
...
pub struct Param<P: Phase = Surface> {
    ...
    pub ty: Option<Ty>,
    pub borrow: bool,
    // A parameter written as a pattern rather than a name, `fn area(Circle(r))`.
    // The parameter still binds one value under `name`, an unforgeable synthetic
    // taken from its position, and desugaring matches the pattern against that
    // name around the body, so the pattern is gone before the checker. Like a
    // `let` pattern it must be irrefutable, and a refutable one is reported the
    // same way: as a match that does not cover every case.
    pub pat: Option<S<Pattern>>,
...
// The synthetic name of a pattern parameter carries no information the pattern
// does not, so the debug dump prints whichever of the two the source wrote.
impl<P: Phase + fmt::Debug> fmt::Debug for Param<P> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut d = f.debug_struct("Param");
        match &self.pat {
            Some(p) => d.field("pat", p),
            None => d.field("name", &self.name),
        };
        d.field("ty", &self.ty);
        d.field("borrow", &self.borrow);
        d.field("default", &self.default).finish()
    }
}
...
pub struct Arm<P: Phase = Surface> {
    ...
    pub body: S<Expr<P>>,
    // Set on every arm but the first of a run produced by splitting one
    // or-pattern. The run stands for a single arm the user wrote, so
    // reachability is judged for the run as a whole: `A => x` followed by
    // `A | B => y` leaves `A | B` reachable through `B`, and reporting the
    // alternative that `A` shadows as an unreachable arm would be a lie.
    // Exhaustiveness is unaffected, since every alternative still contributes.
    pub alt: bool,
}

// An absent guard, and the common case of an arm that is not an or-pattern
// alternative, are omitted from the debug dump.
...
pub enum Pattern {
    ...
    // Alternation, `A | B | C`. Every alternative must bind the same names, and
...
pub fn call<P: Phase>(f: S<Expr<P>>, args: Vec<S<Expr<P>>>, span: Span) -> S<Expr<P>> {
...
/// governs both the source formatter (via `fmt`) and the compiler's type
/// displays, so the two never disagree about the empty row.
pub const SHOW_EMPTY_EFFECT_ROW: bool = false;
