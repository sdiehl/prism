...
use crate::ast::SHOW_EMPTY_EFFECT_ROW;
...
pub fn fmt_labels(ls: &[EffLabel]) -> String {
...
pub fn fmt_data(d: &DataDecl) -> String {
...
impl Fmt<'_> {
    ...
    pub(super) fn fmt_param(&self, p: &Param) -> String {
        ...
        // A pattern parameter's name is synthesized from its position, so the
        // pattern is the only half of it the source wrote.
        let head = p
            .pat
            .as_ref()
            .map_or_else(|| p.name.clone(), fmt_pat_inline);
        let base = p.ty.as_ref().map_or_else(
            || format!("{pre}{head}"),
            |t| format!("{pre}{head} : {}", fmt_ty(t)),
        );
        match &p.default {
            Some(d) => format!(
                "{base} {} {}",
