...
impl Fmt<'_> {
    ...
    // then one `|> stage` per line at `OP_NEST` when the pipeline breaks. The
    // head sits at the pipe's own level and each stage one level tighter, so a
    // looser operand keeps its parens rather than reparsing into the chain.
    fn pipe_doc(&self, x: &S<Expr>, f: &S<Expr>, base: usize) -> Option<Doc> {
        ...
        stages_rev.reverse();
        let inner = base + OP_NEST;
        let head_paren = needs_paren_at(&head.node, Level::Pipe);
        ...
        for stage in stages_rev {
            parts.push(nest(
                ...
                concat([
                    ...
                    text(kw::PIPE_RIGHT),
                    space(),
                    self.operand(stage, inner, needs_paren_at(&stage.node, Level::Compose))?,
                ]),
            ));
        }
        Some(group(concat(parts)))
