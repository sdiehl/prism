...
impl Fmt<'_> {
    ...
    pub(super) fn fmt_expr_inline(&self, e: &S<Expr>, mode: Mode) -> Option<String> {
        match &e.node {
            ...
            Expr::Pipe(x, f) => {
                ...
                let f_s = self.fmt_expr_inline(f, mode)?;
                let x_s = paren_if(needs_paren_at(&x.node, Level::Pipe), x_s);
                let f_s = paren_if(needs_paren_at(&f.node, Level::Compose), f_s);
    ...
    fn fmt_sugar_inline(&self, s: &Sugar<Surface>, mode: Mode) -> Option<String> {
        match s {
            // `??` is right associative, so its left operand sits one level
            // tighter and a left-nested `a ?? b` keeps its parens.
            Sugar::Default(a, b) => {
            ...
            Sugar::Compose(forward, f, g) => {
                ...
                let f_s = paren_if(needs_paren_at(&f.node, Level::Compose), f_s);
                let g_s = paren_if(needs_paren_at(&g.node, Level::Tight), g_s);
                let op = if *forward {
                    kw::COMP_RIGHT
                } else {
                    kw::COMP_LEFT
