...
// The wildcard, which lexes as an ordinary identifier and so can be written
...
// The two prelude `Plate` methods a derived instance defines: one layer of a
// value's same-type children, and the inverse that puts a layer back from
// replacement children. Every whole-tree traversal and rewrite in the library is
// written once against this pair, so the derivation and the prelude class agree
// here.
pub const CHILDREN_METHOD: &str = "children";
pub const REBUILD_METHOD: &str = "rebuild";
...
// The JSON (`lib/std/Json.pr`) class methods a derived instance defines, the two
// tree constructors it builds a tagged object out of, and the member lookup its
// decoder reads a field through. Pinned to Json.pr by the drift-guard test, as
// the wire names above are pinned to Wire.pr.
pub const TO_JSON_METHOD: &str = "to_json";
pub const FROM_JSON_METHOD: &str = "from_json";
pub const JSON_OBJ: &str = "JObj";
pub const JSON_STR: &str = "JStr";
pub const JSON_FIELD_FN: &str = "json_field";
// The key a derived JSON object carries its constructor name under. Field names
// lex as identifiers, so `$` cannot collide with one, which is the same
// unforgeability argument the mangled names above rest on. A single-constructor
// type carries no tag at all: there is nothing to discriminate.
pub const JSON_TAG_KEY: &str = "$";
// The key prefix a positional (non-record) constructor's arguments are named by,
// in declaration order: `_0`, `_1`. A record constructor uses its declared field
...
// The two separators a canonical top-level name is built from: `.` before an
// exported name, `@` before a module-private one. Neither is spellable in a
// source identifier, which is what makes both unforgeable.
const MODULE_SEP: char = '.';
const PRIVATE_SEP: char = '@';
const NAME_SEPS: [char; 2] = [MODULE_SEP, PRIVATE_SEP];
...
pub fn module_of(canon: &str) -> &str {
    canon.rsplit_once(NAME_SEPS).map_or("", |(m, _)| m)
}
...
pub fn private(module: &str, name: &str) -> String {
    format!("{module}{PRIVATE_SEP}{name}")
}

// Whether a canonical top-level name is module-private, and so unspellable from
// another module. The structural inverse of `private`: the `@` it introduces is
// the only way a top-level symbol can carry one. A diagnostic that suggests a
// name to call must filter these out, since no source can name them.
#[must_use]
pub fn is_private(canon: &str) -> bool {
    canon.contains(PRIVATE_SEP)
}

// A derived `Plate` traversal helper: one module-private top-level function per
// distinct field shape a `children` body walks through (a list of nodes, an
// optional node, a nested carrier). Named from the derived type's canonical name
// plus the helper's discovery index, so it lands in the type's own module and two
// types in one module cannot collide. Neither `@` nor `$` is spellable in source,
// so no user name can forge one; `module_of` still recovers the module, because
// the index is separated by `$` rather than a second `@`.
#[must_use]
pub fn plate_helper(ty: &str, index: usize) -> String {
    private(module_of(ty), &format!("plate${}${index}", bare_name(ty)))
}

// The same, for the `rebuild` direction: one helper per shape, putting a shape
// back together from the replacement children it consumes off the front of the
// list. Distinct from `plate_helper` at the same index because the two walk the
// same shape in opposite directions.
#[must_use]
pub fn plate_rebuilder(ty: &str, index: usize) -> String {
    private(module_of(ty), &format!("replate${}${index}", bare_name(ty)))
}

// The module a prepended prelude's definitions belong to. The prelude arrives as
// a text prefix rather than an imported file, so it has no dotted path of its
// own; this stands in as one. A module path is a dotted run of capitalized
// identifiers, so no source can spell `@prelude`, and `private` gives its
// definitions the same unforgeable `Module@name` shape every other module's
// privates get.
//
// A prelude definition keeps its bare name until the user's own file defines
// that name too. Only then does the prelude's move to `@prelude@name`, so the
// user's definition can take the bare name without capturing the prelude's
// internal references to it. Materializing the symbol only at a collision is
// what leaves every non-colliding program's content hash untouched.
pub const PRELUDE_MODULE: &str = "@prelude";

#[must_use]
pub fn prelude_private(name: &str) -> String {
    private(PRELUDE_MODULE, name)
}
...
// keyword argument.
#[must_use]
pub fn pat_param(i: usize) -> String {
    format!("arg@{i}")
}
...
mod tests {
    ...
    // imported module's privates use, invert through the shared accessors, and
    // sit at a path no source can spell.
    #[test]
    fn shadowed_prelude_names_are_module_privates_at_an_unspellable_path() {
        let n = super::prelude_private("mod");
        assert_eq!(n, super::private(super::PRELUDE_MODULE, "mod"));
        assert_eq!(module_of(&n), super::PRELUDE_MODULE);
        assert_eq!(bare_name(&n), "mod");
        assert!(is_synthesized(&n));
        // A module path is dotted capitalized identifiers, so the leading `@`
        // keeps this disjoint from every real module's privates.
        assert!(!super::PRELUDE_MODULE.starts_with(|c: char| c.is_ascii_alphanumeric() || c == '_'));
        assert_ne!(n, super::private("Prelude", "mod"));
    }
    ...
    fn library_hook_names_resolve_to_module_definitions() {
        ...
        // The JSON pair: two class methods, the member lookup a derived decoder
        // reads a field through, and the two tree constructors a derived encoder
        // builds a tagged object out of.
        let json = include_str!("../../../lib/std/Json.pr");
        for m in [TO_JSON_METHOD, FROM_JSON_METHOD] {
            assert!(
                json.contains(&format!("{m} :")),
                "class method `{m}` (names) has no `{m} :` signature in Json.pr"
            );
        }
        assert!(
            json.contains(&format!("fn {JSON_FIELD_FN}(")),
            "member lookup `{JSON_FIELD_FN}` (names) has no `fn {JSON_FIELD_FN}(` in Json.pr"
        );
        for c in [JSON_OBJ, JSON_STR] {
            assert!(
                json.contains(&format!("{c}(")),
    ...
    // A derived `Plate` helper stays in its type's module, keeps two types in one
    // module apart, and is unforgeable (no source name can contain `$`). A
    // root-module type gets the bare form rather than an empty module prefix.
    #[test]
    fn plate_helper_names_are_per_type_and_unforgeable() {
        assert_eq!(
            plate_helper("Syntax.Ast.Expr", 3),
            "Syntax.Ast@plate$Expr$3"
        );
        assert_eq!(module_of(&plate_helper("Syntax.Ast.Expr", 3)), "Syntax.Ast");
        assert_ne!(
            plate_helper("Syntax.Ast.Expr", 0),
            plate_helper("Syntax.Ast.Pat", 0)
        );
        assert_eq!(plate_helper("Tree", 0), "@plate$Tree$0");
        assert_eq!(module_of(&plate_helper("Tree", 0)), "");
        assert!(is_synthesized(&plate_helper("Tree", 0)));
        // The two directions over one shape are separate definitions.
        assert_ne!(plate_helper("Tree", 0), plate_rebuilder("Tree", 0));
        assert!(is_synthesized(&plate_rebuilder("Tree", 0)));
    }
