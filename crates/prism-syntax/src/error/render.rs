...
// unless told otherwise: on a source holding any multi-byte character before the
// span, a character-indexed lookup drifts the caret one column per extra byte,
// and once the offset passes the character count it drops the label and leaves a
// header with nothing under it.
fn report_config(color: bool) -> Config {
    Config::default()
        .with_color(color)
