...
pub enum ErrKind {
    ...
    #[error(
        "field `{field}` of type `{type_name}` has conflicting types across its constructors: \
         ...
         one type, so shared field names must agree"
    )]
    ...
    #[error(
        "record pattern for `{ctor_name}` omits field(s) {missing}; list every field, or add \
         `..` to ignore the rest"
    )]
    IncompleteRecordPattern { ctor_name: String, missing: String },
    // --- effects ---
    ...
    #[error(
        "cannot derive {class} for {ty}; derivable are Eq, Ord, Show, Hash, \
         Serialize, Stable, Arbitrary, ToJson, FromJson, Plate, Lens"
    )]
    NotDerivable { class: String, ty: String },
    #[error(
        "cannot derive Plate for {ty}: {field} reaches `{reached}`, which cannot be traversed. \
         A derived traversal walks lists, optionals, tuples, and the data types declared \
         alongside `{ty}`; it cannot see inside a function, a built-in container, or a \
         recursive occurrence at different type arguments."
    )]
    PlateNotTraversable {
        ty: String,
        field: String,
    ...
    #[error("alternatives of an or-pattern must bind the same names: `{name}` is bound in some alternatives but not all")]
    OrPatternBinders { name: String },
    #[error("or-pattern expands to more than {limit} match arms: split the nested alternations into separate arms")]
    OrPatternTooLarge { limit: usize },
...
impl ErrKind {
    ...
    pub const fn code(&self) -> &'static str {
        match self {
            ...
            Self::ConflictingFieldType { .. } => "E4008",
            Self::IncompleteRecordPattern { .. } => "E4009",
            ...
            Self::OrPatternBinders { .. } => "E6068",
            Self::OrPatternTooLarge { .. } => "E6069",
...
mod tests {
    ...
    fn errkind_codes_are_unique() {
        let kinds: Vec<ErrKind> = every_errkind![
            ...
            IncompleteRecordPattern { ctor_name, missing },
            ...
            OrPatternBinders { name },
            OrPatternTooLarge { limit },
            WithNotLast,
