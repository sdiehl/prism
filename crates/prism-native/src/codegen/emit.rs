...
#[cfg(feature = "mlir")]
use prism_common::{ASCII_PRINTABLE_HI, ASCII_PRINTABLE_LO};
#[cfg(feature = "mlir")]
...
impl<'a, I: Isa> Cg<'a, I> {
    ...
    fn lam_fn(&mut self, idx: usize) -> Result<String, String> {
        ...
        for (i, p) in params.iter().enumerate() {
            let r = format!("%{}", prism_syntax::names::generated_param(i));
            regs.insert(*p, r.clone());
            all_params.push(r);
        }
...
pub(super) fn escape_str(s: &str) -> String {
    ...
    for b in s.bytes() {
        match b {
            ...
            ASCII_PRINTABLE_LO..=ASCII_PRINTABLE_HI => escaped.push(b as char),
