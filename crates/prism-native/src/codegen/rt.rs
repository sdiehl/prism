...
//! are not here: their symbols are derived once from [`prism_core::core::builtins::Builtin::sym`].
//!
...
/// Bare compiler name resolved on `PATH` at exec time when no absolute toolchain
/// can be located on disk. Every distro's clang package puts a `clang` here.
const DEFAULT_CC_NAME: &str = "clang";

/// Absolute `clang` locations probed, in order, when the build-baked default is an
/// absolute path that does not exist on the host. A single release binary ships to
/// several distros whose clang layouts differ: Debian/Ubuntu (apt.llvm.org) install
/// it under the versioned toolchain dir, while Fedora, Arch, and Alpine put it at
/// `/usr/bin/clang`; macOS keeps it under the Homebrew LLVM prefix. The first entry
/// mirrors the release build's baked path so an in-place Debian install is unchanged.
const CC_FALLBACKS: &[&str] = &[
    "/usr/lib/llvm-22/bin/clang", // Debian/Ubuntu (apt.llvm.org snapshot)
    "/usr/bin/clang",             // Fedora, Arch, Alpine, generic Linux
    "/opt/homebrew/opt/llvm/bin/clang", // macOS Homebrew (Apple Silicon)
    "/usr/local/opt/llvm/bin/clang", // macOS Homebrew (Intel)
];

/// The C compiler to invoke: `$PRISM_CC`, else the build-configured default
/// (`env!("PRISM_BUILD_CC")`) resolved for the host it actually runs on.
#[must_use]
pub fn cc() -> String {
    std::env::var("PRISM_CC").unwrap_or_else(|_| resolve_build_cc())
}

/// Resolve the build-configured default compiler against the current host.
///
/// A bare command name (dev builds default to `clang`) is left for the OS to
/// resolve on `PATH` at exec time. An absolute path is distro-specific: the release
/// binary bakes in the path that existed on the build runner, which may be absent on
/// the host it was installed on. When that baked path is missing we probe known
/// locations before giving up on a bare `clang`, so a release binary links out of
/// the box on every distro rather than only the one it was built on.
fn resolve_build_cc() -> String {
    let baked = env!("PRISM_BUILD_CC");
    if !baked.contains('/') || Path::new(baked).exists() {
        return baked.to_string();
    }
    CC_FALLBACKS
        .iter()
        .find(|cand| Path::new(cand).exists())
        .map_or_else(|| DEFAULT_CC_NAME.to_string(), |cand| (*cand).to_string())
}
...
pub const ALLOC: &str = "prism_alloc";
pub const REUSE_ALLOC: &str = "prism_reuse_alloc";
pub const REUSE_TOKEN: &str = "prism_reuse_token";
...
pub const PRINT_FLOAT: &str = "prism_print_float";
pub const PRINT_NL: &str = "prism_print_nl";
pub const READ_INT: &str = "prism_prim_read_int";
pub const READ_LINE: &str = "prism_prim_read_line";
pub const RAND: &str = "prism_prim_rand";
pub const SRAND: &str = "prism_srand";
pub const FATAL: &str = "prism_fatal";
pub const ERROR_INT: &str = "prism_error_int";

// Local mutable cells (the runtime form of an escape-checked `var`).
pub const REF_NEW: &str = "prism_ref_new";
pub const REF_GET: &str = "prism_ref_get";
pub const REF_SET: &str = "prism_ref_set";

// Bignum arithmetic, the slow path of the tagged-int operators.
pub const INT_ADD: &str = "prism_rt_int_add";
pub const INT_SUB: &str = "prism_rt_int_sub";
pub const INT_MUL: &str = "prism_rt_int_mul";
pub const INT_DIV: &str = "prism_rt_int_div";
pub const INT_REM: &str = "prism_rt_int_rem";
pub const INT_CMP: &str = "prism_rt_int_cmp";
...
pub const INT_OF_LONG: &str = "prism_int_of_long";

// Effect-machine instrumentation and the two miscompile traps: the
// over-application trap and the non-exhaustive-`case` trap.
pub const EFFOP_ALLOC: &str = "prism_effop_alloc";
pub const DRIVE_STEP: &str = "prism_drive_step";
pub const APPLY_ERROR: &str = "prism_apply_error";
pub const MATCH_ERROR: &str = "prism_match_error";

// Native kont shadow-state instrumentation.
pub const NATIVE_KONT_ENTER: &str = native_kont::ENTER_SYMBOL;
pub const NATIVE_KONT_ARG: &str = native_kont::ARG_SYMBOL;
pub const NATIVE_KONT_TAILCALL: &str = native_kont::TAILCALL_SYMBOL;
pub const NATIVE_KONT_LEAVE: &str = native_kont::LEAVE_SYMBOL;
...
mod tests {
    ...
    const KONT_TEST_CORE_NAME: &str = prism_syntax::names::ENTRY_POINT;
    const KONT_LOOKUP_HARNESS: &str =
        include_str!("../../../../tests/fixtures/native_kont_lookup.c");
