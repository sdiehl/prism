...
use super::super::cbpv::{Comp, Core, CorePat, Value};
use super::super::fv::{comp as freev, comp_without, pat_vars};
use super::super::tailrec::{loops_as_tail_call, reassoc, trmc_shape};
...
pub fn infer_borrow_sigs(core: &Core, pure_fns: &Set, declared: &Sigs) -> Sigs {
    ...
    let bodies: Vec<Comp> = core.fns.iter().map(|f| reassoc(&f.body)).collect();
    ...
    fn comp(&mut self, comp: &Comp, loans: &Loans, unowned: &Unowned, tail: bool) {
        match comp {
            ...
            Comp::Bind(head, binder, rest) => {
                ...
                let mut rest_loans = loans.clone();
                let mut rest_unowned = unowned.clone();
                ...
                if let Comp::Return(v) = head.as_ref() {
                    match v {
                        Value::Var(x) => {
                            if let Some(root) = loans.get(x) {
                    ...
                    self.comp(head, loans, unowned, false);
                }
                self.comp(rest, &rest_loans, &rest_unowned, tail);
            }
            ...
            Comp::If(cond, yes, no) => {
                ...
                self.comp(yes, loans, unowned, tail);
                self.comp(no, loans, unowned, tail);
            }
            Comp::Prim(_, lhs, rhs) => {
            ...
            Comp::Case(scrutinee, arms) => {
                ...
                let scrutinee_unowned = matches!(scrutinee, Value::Var(x) if unowned.contains(x));
                for (pattern, body) in arms {
                    ...
                    let mut arm_loans = loans.clone();
                    let mut arm_unowned = unowned.clone();
                    ...
                    self.comp(body, &arm_loans, &arm_unowned, tail);
                }
            }
...
fn fitting_alloc(comp: &Comp, cap: usize) -> bool {
    match comp {
        Comp::Return(v @ (Value::Ctor(..) | Value::Tuple(..))) => {
            let arity = match v {
                Value::Ctor(_, _, fields) | Value::Tuple(fields) => fields.len(),
                _ => 0,
            };
            arity <= cap
                && !matches!(v, Value::Ctor(name, ..) if kw::is_or_null_ctor(name.as_str()))
        }
        Comp::Bind(m, _, n) => fitting_alloc(m, cap) || fitting_alloc(n, cap),
        Comp::If(_, yes, no) => fitting_alloc(yes, cap) || fitting_alloc(no, cap),
        Comp::Case(_, arms) => arms.iter().any(|(_, body)| fitting_alloc(body, cap)),
        Comp::WithReuse { body, .. } => fitting_alloc(body, cap),
        _ => false,
    }
}
...
#[cfg(test)]
mod tests {
    use super::super::super::cbpv::{CoreFn, CorePat};
    use super::*;
    ...
    fn structured_argument_at_a_call_site_forces_owned() {
        ...
        let caller = f(
            ...
            Comp::Call(
                s("reader"),
                vec![Value::Ctor(s("Node"), 0, vec![Value::Var(s("n"))])],
            ),
        );
        let sigs = infer_borrow_sigs(
    ...
    fn returned_and_stored_params_stay_owned() {
        ...
        let stored = f(
            "stored",
            &["x"],
            Comp::Return(Value::Ctor(s("Box"), 0, vec![Value::Var(s("x"))])),
        );
        let sigs = infer_borrow_sigs(
            &core(vec![ret, stored]),
            &pure_set(&["ret", "stored"]),
