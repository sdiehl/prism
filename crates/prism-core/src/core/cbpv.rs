...
// without appearing as a call head. A body's free variables are taken modulo the
// function's own parameters: a parameter that happens to share a top-level name
// shadows it, and counting it as a reference would keep dead code alive and, via
...
mod reachability_tests {
    ...
    fn function_of(name: &str, params: &[&str], body: Comp) -> CoreFn {
        CoreFn {
            params: params.iter().copied().map(Sym::new).collect(),
            ..function(name, body)
        }
    }
    ...
    #[test]
    fn a_parameter_does_not_reach_a_same_named_function() {
        let shadowed = Sym::new("args");
        let helper = Sym::new("helper");
        let core = Core {
            fns: vec![
                function(ENTRY_POINT, Comp::Call(helper, vec![Value::Int(1)])),
                function_of("helper", &["args"], Comp::Return(Value::Var(shadowed))),
                function("args", Comp::Return(Value::Int(2))),
            ],
        };
        assert_eq!(
            reachable_fns(&core),
