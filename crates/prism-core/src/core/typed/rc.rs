...
fn borrowed_call_vars(comp: &TypedComp, sigs: &Sigs) -> Set {
    ...
    args.iter()
        ...
        // position leaves owned. A borrowed position holding a non-variable
        // (a literal or a freshly built value) has no prior owner to retain,
        // so it is correctly absent from the set: skip it rather than crash.
        // On verified input every borrowed argument is an erased variable, so
        // this filter drops nothing there.
        .filter_map(|(_, arg)| erased_var(arg))
        .collect()
}
