...
impl RowNames {
    // Deliberately not `Iterator`: the supply is infinite and never yields
    // `None`, so the trait's contract would mislead.
    #[allow(clippy::should_implement_trait)]
    pub fn next(&mut self) -> Sym {
...
/// miscompiling.
pub fn strip_resume(c: &TypedComp, aliases: &BTreeSet<Sym>, drift: &DriftLog) -> Option<TypedComp> {
...
pub type Env = BTreeMap<i64, TypedBinder>;
...
impl Retyped {
    ...
    pub fn insert(&mut self, name: Sym, ty: CoreType) -> Option<CoreType> {
