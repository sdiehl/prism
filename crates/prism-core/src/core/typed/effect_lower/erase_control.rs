...
use prism_syntax::names::{self, FOREVER, REPEAT_WHILE};
...
impl StepAt {
    pub const fn new(more: Type, done: Type) -> Self {
        Self { more, done }
    }
    ...
    pub fn sdone(&self, v: TypedValue) -> TypedValue {
        TypedValue::new(
            self.ty(),
            TypedValueKind::Ctor {
                name: Sym::new(SDONE),
    ...
    pub fn more_pattern(&self, binder: TypedBinder) -> TypedPattern {
        TypedPattern::Ctor {
            name: Sym::new(SMORE),
            instantiation: self.instantiation(),
    ...
    pub fn done_pattern(&self, binder: TypedBinder) -> TypedPattern {
        TypedPattern::Ctor {
            name: Sym::new(SDONE),
            instantiation: self.instantiation(),
            fields: vec![Some(binder)],
                ...
                let ty = env.get(name).map_or_else(
                    ...
                    |stored| {
                        if instantiation.is_empty() {
                            ...
                            // Unreachable on valid input: control erasure
                            // preserves every local's instantiability. If the
                            // invariant ever slips, fall back to the value's
                            // existing type (the same well-defined behavior as
                            // the env-miss branch above) rather than crash, so
                            // an imperfect invariant never surfaces as an
                            // observable compiler failure during preparation.
                            instantiate_value_scheme(stored, instantiation).unwrap_or_else(
                                |error| {
                                    debug_assert!(
                                        false,
