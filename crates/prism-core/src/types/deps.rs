...
use prism_syntax::ast::{Core, Decl, Expr, HandlerArm, Pattern, Program, S};
...
#[must_use]
pub fn dep_sccs(prog: &Program<Core>) -> Vec<Vec<usize>> {
...
fn collect_pat_binders<'a>(p: &'a S<Pattern>, bound: &mut Vec<&'a str>) {
    match &p.node {
        ...
        // Every alternative binds the same names, so the first one names them
        // all. (Desugaring splits alternation into separate arms, so this arm
        // is only reached by a caller walking a surface pattern.)
        Pattern::Or(alts) => {
            if let Some(first) = alts.first() {
                collect_pat_binders(first, bound);
            }
        }
        Pattern::Wild
        | Pattern::Int(_)
