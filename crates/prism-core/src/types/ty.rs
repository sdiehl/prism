...
pub use prism_syntax::kind::Kind;
...
// The opt-in JSON conversion classes, defined in `lib/std/Json.pr`. Unlike the
// wire codec, whose format is frozen bytes, these two convert to and from the
// dynamic `Json` tree, so they are derived as a pair and never one without the
// other: a type that encodes but cannot decode is a document nobody can read back.
pub const TO_JSON_CLASS: &str = "ToJson";
pub const FROM_JSON_CLASS: &str = "FromJson";
...
// The one-layer generic-traversal class: `children(x)` is the list of `x`'s
// immediate same-type subvalues, from which whole-tree traversals (universe,
// fold, count, transform) are written once for every type instead of once per
// constructor. Declared in the prelude beside the other structural classes.
pub const PLATE_CLASS: &str = "Plate";
...
pub use prism_syntax::ast::SHOW_EMPTY_EFFECT_ROW;
