...
//! `kont` frame, over the interpreter's runtime state) are distinct formats that
//! stay in their own homes; only the byte machinery lives here: LEB128 varints
//! (unsigned and zigzag-signed), length-bounded blobs and strings, fixed-width
//! scalars, table-index lookups, and the hostile-input discipline (a varint byte
//! cap, a length bound, truncation and range checks) that keeps every decode
//! total.
//!
...
/// The parse failures a hostile or stale `def` frame can produce.
///
/// Decode is total: every malformed input lands on one of these rather than a
/// panic.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum CodecError {
    /// The scheme tag was absent or not the expected hash scheme.
    Scheme,
...
pub const WIRE_LEN_MAX: u64 = 1 << 20;
...
pub const MAX_EXPANSION: usize = 1 << 22;
...
pub fn put_svarint(out: &mut Vec<u8>, x: i64) {
...
pub fn put_indices(out: &mut Vec<u8>, idxs: &[u32]) {
...
/// The table entry a wire number names.
///
/// The inverse of [`to_wire`], and the shared tag-byte-to-variant lookup: a
/// byte out of the table's range is rejected as [`CodecError::Malformed`]
/// rather than misread.
///
/// # Errors
/// [`CodecError::Malformed`] when the number is not a valid index into `table`.
pub fn from_wire<T: Copy>(table: &[T], n: u64) -> Result<T, CodecError> {
    usize::try_from(n)
        .ok()
...
mod tests {
    use super::{
        from_wire, put_str, put_svarint, put_uvarint, to_wire, CodecError, Reader, VARINT_MAX_BYTES,
    };
